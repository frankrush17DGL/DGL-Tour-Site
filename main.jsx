  ...
  return (
    <section className="red-room-page trophies-page trophy-rooms-page">
                  ...
                  <div className="timeline-results">
                    {yearRecords.map(record => (
                      <p key={record.player + record.finish}>
                        <b>{`${String(record.finish).toUpperCase().startsWith('T') ? String(record.finish).toUpperCase() : '#'+String(record.finish)}`}</b>
      ...
      {galleryPhotos.length ? (
        <article className="trophy-gallery-card">
          <p className="eyebrow">Photo Gallery</p>
          <div className="trophy-photo-gallery">
            {galleryPhotos.map((photo, index) => (
              <AssetPhoto
                key={photo + index}
                src={photo}
                alt={`${selectedState} trophy gallery ${index + 1}`}
                className="trophy-gallery-photo"
                fallback="🏆"
              />
            ))}
          </div>
