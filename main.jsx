...
function parseFutureEvents(text) {
  ...
  const findRow = labels => {
    const targets = labels.map(labelFor);
    const index = rows.findIndex(row => targets.includes(labelFor(row[0])));
  ...
  const notesRow = findRow(['Notes', 'Notes/Number of Commits', 'Number of Commits', 'of commits', 'commits']);
  ...
  // Course row is the one that has been failing. If the label is not exported cleanly,
  // infer it as the row immediately after Date and immediately before Time.
  const rCourse = courseRow >= 0
    ? courseRow
    : (dateRow >= 0 && timeRow >= 0 && timeRow > dateRow ? dateRow + 1 : 3);

  const rTime = rowIndex(timeRow, rCourse + 1);
  const rNotes = rowIndex(notesRow, rTime + 1);
  ...
  for (let col = 1; col < maxCol; col++) {
    ...
    let course = textCell(rows[rCourse]?.[col]);
    const time = textCell(rows[rTime]?.[col]);
    const notes = textCell(rows[rNotes]?.[col]);

    // Last-resort course recovery: scan rows between Date and Time for a non-date/non-time value.
    if (!course && dateRow >= 0 && timeRow >= 0 && timeRow > dateRow) {
      for (let row = dateRow + 1; row < timeRow; row++) {
        const candidate = textCell(rows[row]?.[col]);
        if (candidate && !/^\d{1,2}\/\d{1,2}/.test(candidate) && !/^\d{1,2}:\d{2}/.test(candidate)) {
          course = candidate;
          break;
        }
      }
