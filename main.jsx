...
function parseEventColumns(sheet, sheetName = CURRENT_YEAR_SHEET) {
  ...
  function findLabelPosition(label, fallbackRow, fallbackCol = 3) {
    const target = label.toLowerCase();
  ...
  function scanColumnForCourse(col, eventNo, date, tees, time) {
    const badValues = new Set([
      ...
      'tees',
      'time',
      String(eventNo || '').toLowerCase(),
      String(date || '').toLowerCase(),
      String(tees || '').toLowerCase(),
      String(time || '').toLowerCase()
    ]);

    for (let row = 0; row <= 14; row++) {
      const value = cleanCourse(sheet[row]?.[col]);
      const lower = String(value || '').toLowerCase().trim();

      if (!value || badValues.has(lower)) continue;
  ...
  function scanColumnForTime(col) {
    for (let row = 0; row <= 14; row++) {
      const value = cleanTime(sheet[row]?.[col]);
  ...
  const eventPos = findLabelPosition('Event', 1);
  const datePos = findLabelPosition('Date', 2, eventPos.col);
  const coursePos = findLabelPosition('Course', 3, eventPos.col);
  const teesPos = findLabelPosition('Tees', 4, eventPos.col);
  const timePos = findLabelPosition('Time', 5, eventPos.col);
  ...
  for (let col = startCol; col <= maxCol; col++) {
    ...
    let course = cleanCourse(sheet[coursePos.row]?.[col]);
    const tees = cleanTees(sheet[teesPos.row]?.[col]);
    let time = cleanTime(sheet[timePos.row]?.[col]);

    if (!course) {
      course = scanColumnForCourse(col, eventNo, date, tees, time);
    }

    if (!time) {
      time = scanColumnForTime(col);
    }

    if (sheetName === CURRENT_YEAR_SHEET && String(eventNo) === '5' && String(date).trim() === '6/20') {
      course = course || 'Troy Burne';
      time = time || '8:00 AM';
    }
...
function parseSportsbook(text) {
  ...
  const headerIndex = rows.findIndex(row =>
    ...
    row.some(cell => ['dglrating', 'powerrating', 'powerscore'].includes(normalizeHeader(cell)))
  );
  ...
  const indexes = {
    ...
    ceiling: indexOf(['ceiling', 'redroom', 'performanceindex']),
    attendance: indexOf(['attendance', 'eventplayed', 'eventsplayed']),
    pedigree: indexOf(['pedigree']),
    ghin: indexOf(['currentghin', 'latesthandicap', 'ghin']),
    rating: indexOf(['dglrating', 'powerrating', 'powerscore']),
  ...
  return rows.slice(headerIndex + 1)
    .map(row => {
      ...
      return {
        ...
        rating: indexes.rating >= 0 ? numberFromCell(row[indexes.rating]) : 0,
        winPercent: indexes.winPercent >= 0 ? parsePercentCell(row[indexes.winPercent]) : 0,
        odds: indexes.odds >= 0 ? String(row[indexes.odds] || '').trim() : '',
...
function sportsbookInsights(board = []) {
  ...
  const highestCeiling = [...sorted].sort((a, b) => b.ceiling - a.ceiling)[0] || {};
  const ghinMonster = [...sorted].filter(player => player.ghin).sort((a, b) => a.ghin - b.ghin)[0] || {};
  return { favorite, bestValue, highestCeiling, ghinMonster };
}
    ...
    <section className="red-room-page sportsbook-page">
      ...
      <div className="red-page-layout">
        ...
        <article className="tier-card">
          ...
          <div className="tier-row"><strong>🚀 Highest Ceiling</strong><span>{insights.highestCeiling.player || '—'}</span><em>{insights.highestCeiling.ceiling || '—'}</em></div>
          <div className="tier-row"><strong>🎯 Low GHIN Threat</strong><span>{insights.ghinMonster.player || '—'}</span><em>{insights.ghinMonster.ghin || '—'}</em></div>
        </article>
...
function StateTrophiesPage({ goHome }) {
  const trophies = [
    {
      state: 'Florida',
      title: 'Shootout in da Swamp',
      description: 'Florida state trophy history from Regatta Bay, Oak Marsh, TPC Sawgrass Dye’s Valley, and Kelly Plantation.',
      results: [
        {
          year: '2021',
          courses: 'Regatta Bay',
          places: [
            '1st Place: Frank Rush & Keegan Anderson (tie)',
            '2nd Place: Charlie Kordonowy',
            '3rd Place: John Leitch'
          ]
        },
        {
          year: '2023',
          courses: 'Oak Marsh & TPC Sawgrass (Dye’s Valley Course)',
          places: [
            '1st Place: Charlie Kordonowy',
            '2nd Place: Frank Rush',
            '3rd Place: Max Olson',
            '4th Place: Witt Weldon',
            '5th Place: Patrick Sheehy',
            '6th Place: Alex Pletsch'
          ]
        },
        {
          year: '2026',
          courses: 'Kelly Plantation',
          places: [
            '1st Place: Frank Rush',
            '2nd Place: Max Olson'
          ]
        }
      ]
    ...
    {
      state: 'Arizona',
      title: 'Arizona Desert Classic',
      description: 'Arizona state trophy history from desert golf trips and Troon North Pinnacle.',
      results: [
        {
          year: '2023',
          courses: 'Troon North Pinnacle',
          places: [
            'Winner: Frank Rush',
            '2nd Place: Alex Rogers'
          ]
        }
      ]
    }
          ...
          <article className="red-full-table" key={trophy.state}>
            ...
            <div className="trophy-scroll">
              {trophy.results.map(result => (
                <div className="trophy-photo-placeholder" key={trophy.state + result.year}>
                  <strong>{result.year}</strong>
                  <small>{result.courses}</small>
