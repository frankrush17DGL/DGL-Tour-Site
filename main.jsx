...
function parseYearStandings(sheet) {
  const headerRows = sheet.slice(0, 18);
  let totalCol = -1;
  let rankCol = -1;
  ...
  headerRows.forEach(row => {
    row.forEach((cell, col) => {
      const h = normalizeHeader(cell);
      if (['totaldglpoints', 'dglfcpoints', 'totalpoints', 'seasonpoints', 'points'].includes(h) && col > 0) totalCol = col;
      if (h === 'rank' && col > 0) rankCol = col;
    });
  });

  const blocks = extractPlayerBlocks(sheet);
  const pointLabels = ['Total DGL Points', 'DGLFC Points', 'Total Points', 'Season Points', 'Points'];
  const rankLabels = ['Rank', 'Regular Season Rank', 'DGL Rank'];

  const players = blocks.map(block => {
    const pointsRow = findRowInBlockAny(block, pointLabels);
    const rankRow = findRowInBlockAny(block, rankLabels);

    const points = pointsRow.length
      ? (totalCol >= 0 && Number.isFinite(numberFromCell(pointsRow[totalCol])) && numberFromCell(pointsRow[totalCol]) !== 0
        ? numberFromCell(pointsRow[totalCol])
        : lastNumericValue(pointsRow, 1))
      : 0;

    const rank = rankRow.length
      ? (rankCol >= 0 && Number.isFinite(numberFromCell(rankRow[rankCol])) && numberFromCell(rankRow[rankCol]) !== 0
        ? numberFromCell(rankRow[rankCol])
        : lastNumericValue(rankRow, 1))
      : 999;
