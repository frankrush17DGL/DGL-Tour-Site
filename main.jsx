        ...
        <article className="red-full-table sportsbook-board">
          ...
          <div className="red-table-full">
            {board.length ? board.map((player, index) => (
              <div className="sportsbook-card-row" key={player.player}>
                <div className="sportsbook-rank">#{index + 1}</div>
                <div className="sportsbook-name-block">
                  <strong>{player.player}</strong>
                  <small>{valueTierLabel(player.valueRating)}</small>
                </div>
                <div className="sportsbook-metric">
                  <span>Rating</span>
                  <b>{player.rating ? Number(player.rating).toFixed(1) : '—'}</b>
                </div>
                <div className="sportsbook-metric">
                  <span>Win %</span>
                  <b>{formatPercent(player.winPercent)}</b>
                </div>
                <div className="sportsbook-odds">{formatOdds(player.odds)}</div>
                <div className={'sportsbook-move ' + movementClass(player)}>{movementDisplay(player)}</div>
              </div>
            )) : (
...
function PlayerTradingCard({ profile }) {
  ...
  function headlineBadge(item) {
    ...
    if (item.redRoomCount > 0) return 'RED ROOM ROUND';
    if (item.isRookie) return 'ROOKIE CARD';
    return 'OFFICIAL TOUR CARD';
  }
  ...
  const rankText = profile.rank || cardNo;

  return (
    <button
      className={'gum83-card ' + (flipped ? 'is-flipped' : '')}
      aria-label={`Flip ${profile.name} trading card`}
      onClick={() => setFlipped(!flipped)}
      style={{ '--card-color': cardColor }}
      type="button"
    >
      <div className="gum83-inner">
        <div className="gum83-face gum83-front">
          <div className="gum83-print">
            <div className="gum83-top">
              <div className="gum83-logo"><b>DGL</b><span>TOUR</span></div>
              <div className="gum83-series">1970s WAX PACK SERIES</div>
              <div className="gum83-number">#{rankText}</div>
            </div>

            <div className="gum83-photo-wrap">
              <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="gum83-action-photo" />
              <div className="gum83-photo-wash" />
            </div>

            <div className="gum83-name-banner">
              <div className="gum83-headshot-disc">
                <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="gum83-headshot" />
              </div>
              <h2>{profile.name}</h2>
              <p>{title}</p>
            </div>

            <div className="gum83-stats-row">
              <div><span>RANK</span><b>{profile.rank || '—'}</b></div>
              <div><span>PTS</span><b>{profile.points != null ? profile.points : '—'}</b></div>
              <div><span>RED</span><b>{profile.redRoomCount || 0}</b></div>
            </div>

            <div className="gum83-bottom-ribbon">{achievementLine(profile)}</div>
          </div>
        </div>

        <div className="gum83-face gum83-back">
          <div className="gum83-back-print">
            <div className="gum83-back-masthead">
              <div className="gum83-back-logo">DGL</div>
              <div>
                <span>OFFICIAL PLAYER RECORD</span>
                <h3>{profile.fullName || profile.name}</h3>
                <p>{title} • Card #{cardNo}</p>
              </div>
            </div>

            <div className="gum83-back-grid">
              <section>
                <h4>CAREER STATS</h4>
                <dl>
                  ...
                  <div><dt>State Trophies</dt><dd>{profile.stateTrophyWins || 0}</dd></div>
              ...
              <section>
                ...
                <dl>
                  ...
                  <div><dt>Hometown</dt><dd>{profile.hometown || '—'}</dd></div>
