  ...
  return rows.slice(headerIndex + 1)
    .map(row => {
      ...
      return {
        ...
        valueLabel: indexes.valueRating >= 0 ? String(row[indexes.valueRating] || '').trim() : ''
      };
    })
    .filter(Boolean)
...
function sportsbookInsights(board = []) {
  ...
  const bestValue = sorted.find(player => /best value|value/i.test(String(player.valueLabel || ''))) || sorted[1] || {};
...
function normalizeValueLabel(value, player) {
  const raw = String(value || '').trim();
  const clean = raw.replace(/^[^A-Za-z]+/, '').trim().toLowerCase();

  if (clean.includes('best value')) return '🟢 Best Value';
  if (clean === 'value' || clean.includes(' value')) return '✅ Value';
  if (clean.includes('overpriced')) return '🔴 Overpriced';
  if (clean.includes('avoid')) return '❌ Avoid';
  if (clean.includes('fair')) return '⚖️ Fair';

  const winPct = Number(player?.winPercent || player?.winPct || player?.win || 0);
  return winPct <= 0 ? '❌ Avoid' : '⚖️ Fair';
}
...
function valueTierClass(value, player) {
  const label = normalizeValueLabel(value, player).toLowerCase();
  if (label.includes('best value')) return 'strong';
  if (label.includes('value')) return 'value';
  if (label.includes('overpriced') || label.includes('avoid')) return 'over';
  return 'fair';
}
      ...
      <article className="dgl84-odds-board">
        ...
        <div className="dgl84-odds-table">
          ...
          {board.length ? board.map((player, index) => {
            ...
            const tier = normalizeValueLabel(player.valueLabel, player);
            const tierClass = valueTierClass(player.valueLabel, player);
            return (
            ...
            <p className="note">Sportsbook board not found yet. Make sure the Power Model tab has Player, DGL Rating, Win %, Odds, Movement, Last Week Odds, and Value headers.</p>
          )}
        </div>
      </article>
    </section>
