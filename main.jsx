...
function parseFutureEvents(text) {
  ...
  const firstDataColumn = (() => {
    // Future Events is intentionally maintained left-to-right with labels in column A.
    // Start reading events in column B no matter how Google exports hidden/empty cells.
    return 1;
  })();
  ...
  const findRow = labels => {
    ...
    for (let r = 0; r < Math.min(rows.length, 20); r++) {
      for (let c = 0; c < Math.min(rows[r].length, 3); c++) {
        const cell = normalizeLabel(rows[r][c]);
        if (targets.includes(cell)) return r;
      }
    }
  ...
  const courseRow = findRow(['Course']);
  const timeRow = findRow(['Time']);
  const notesRow = findRow(['Notes', 'Notes Number of Commits', 'Number of Commits', '# of commits', '# of commit', 'commits']);

  const rowIndex = (found, fallback) => found >= 0 ? found : fallback;
  ...
  const looksLikeTime = value => /\b\d{1,2}:\d{2}\b/.test(String(value || '')) || /\b(am|pm)\b/i.test(String(value || ''));
  const looksLikeDate = value => /^\d{1,2}\/\d{1,2}(?:\/\d{2,4})?$/.test(String(value || '').trim()) || /202\d/.test(String(value || ''));
  const looksLikeWeek = value => /^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}/.test(String(value || '').trim());
  const looksLikeEventNumber = value => /^\d+(?:\.0)?$/.test(String(value || '').trim());
  const looksLikeNotes = value => /commit|major|optional|tentative|open|spots?/i.test(String(value || ''));

  function courseForColumn(col) {
    const direct = textCell(rows[rCourse]?.[col]);
    if (direct) return direct;

    // Fallback 1: course should be the row between Date and Time.
    if (rDate >= 0 && rTime >= 0 && rTime > rDate) {
      for (let r = rDate + 1; r < rTime; r++) {
        const candidate = textCell(rows[r]?.[col]);
        if (candidate && /[A-Za-z]/.test(candidate)) return candidate;
      }
    }

    // Fallback 2: scan the first 12 rows in this event column for the most course-like value.
    for (let r = 0; r < Math.min(rows.length, 12); r++) {
      if ([rWeek, rEvent, rDate, rTime, rNotes].includes(r)) continue;
      const candidate = textCell(rows[r]?.[col]);
      if (!candidate) continue;
      if (!/[A-Za-z]/.test(candidate)) continue;
      if (looksLikeTime(candidate) || looksLikeDate(candidate) || looksLikeWeek(candidate) || looksLikeEventNumber(candidate) || looksLikeNotes(candidate)) continue;
      if (normalizeLabel(candidate) === 'course') continue;
      return candidate;
    }
  ...
  const maxCol = Math.max(
    ...rows.slice(0, Math.min(rows.length, 12)).map(row => row.length),
    0
  );

  const events = [];

  for (let col = firstDataColumn; col < maxCol; col++) {
    const week = textCell(rows[rWeek]?.[col]);
    const eventNo = textCell(rows[rEvent]?.[col]);
    const date = textCell(rows[rDate]?.[col]);
    const course = courseForColumn(col);
