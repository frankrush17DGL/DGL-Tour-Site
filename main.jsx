...
function parseFutureEvents(text) {
  ...
  const rows = parseCSV(text).filter(row => row.some(cell => textCell(cell)));
  ...
  rows.forEach((row, rowIndex) => {
    ...
    if (label === 'week') labelRows.week = rowIndex;
    if (['event', 'eventnumber', 'eventno'].includes(label)) labelRows.event = rowIndex;
    if (label === 'date') labelRows.date = rowIndex;
    if (['course', 'coursename'].includes(label)) labelRows.course = rowIndex;
    if (['tees', 'tee'].includes(label)) labelRows.tees = rowIndex;
    if (['time', 'teetime'].includes(label)) labelRows.time = rowIndex;
    if (['notes', 'note'].includes(label)) labelRows.notes = rowIndex;
  });
  ...
  for (let col = 1; col < maxCol; col++) {
    ...
    // Skip blank columns, but keep future events even if tees/notes are blank.
