...
function netNumber(round) { const n = Number(String(round.net || '').replace(/[^\d.-]/g, '')); return Number.isFinite(n) ? n : 0; }
function tierForRound(round) { const net = netNumber(round); if (net <= -6) return 'Elite Table'; if (net <= -4) return 'Excellence Lounge'; return 'Members Gallery'; }
...
function RedRoomPage({ data, goHome }) {
  const featuredRedRound = data.redRounds[0] || {};
  const elite = data.redRounds.filter(round => netNumber(round) <= -6);
  const excellence = data.redRounds.filter(round => netNumber(round) <= -4 && netNumber(round) > -6);
  const gallery = data.redRounds;
  ...
  return (
    <section className="red-room-page">
      ...
      <div className="red-page-layout">
        <article className="royalty-card"><p className="eyebrow">👑 Red Room Royalty</p><h2>{featuredRedRound.player}</h2><strong>Net {featuredRedRound.net}</strong><span>{featuredRedRound.course}</span><small>{featuredRedRound.date} • {featuredRedRound.score}</small></article>
        ...
        <article className="red-full-table">
          <div className="panel-head"><div><p className="eyebrow">All-Time Red Rounds</p><strong>Live Hall of Fame</strong></div><span>{data.redRounds.length} entries</span></div>
          <div className="red-table-full">
            {data.redRounds.map((round, index) => (
              <div className="red-row-full" key={round.player + round.course + round.date + index}><span>#{index + 1}</span><strong>{round.player}</strong><small>{round.course}</small><small>{round.date}</small><em>{round.net}</em><b>{tierForRound(round)}</b></div>
            ))}
          </div>
        </article>
      </div>
