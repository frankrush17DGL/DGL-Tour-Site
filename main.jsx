...
function parseFutureEvents(text) {
  ...
  function findLabelRow(possibleLabels) {
    const wanted = possibleLabels.map(normalizeHeader);
    return rows.findIndex(row => wanted.includes(normalizeHeader(row?.[0])));
  }

  let weekRow = findLabelRow(['Week']);
  let eventRow = findLabelRow(['Event', 'Event #', 'Event Number']);
  let dateRow = findLabelRow(['Date']);
  let courseRow = findLabelRow(['Course']);
  let timeRow = findLabelRow(['Time', 'Tee Time', 'Tee Times']);
  let notesRow = findLabelRow(['Notes', 'Committed', 'Commitments', 'Number of Commits', 'Commits']);

  // Final expected layout on Future Events:
  // Row 1 Week, Row 2 Event, Row 3 Date, Row 4 Course, Row 5 Time, Row 6 Notes.
  // If labels are present but one row is missed, infer the sequence from the Week row.
  if (weekRow !== -1) {
    if (eventRow === -1) eventRow = weekRow + 1;
    if (dateRow === -1) dateRow = weekRow + 2;
    if (courseRow === -1) courseRow = weekRow + 3;
    if (timeRow === -1) timeRow = weekRow + 4;
    if (notesRow === -1) notesRow = weekRow + 5;
  }

  // Absolute fallback for a clean 6-row tab with labels in column A.
  if (eventRow === -1) eventRow = 1;
  if (dateRow === -1) dateRow = 2;
  if (courseRow === -1) courseRow = 3;
  if (timeRow === -1) timeRow = 4;
  if (notesRow === -1) notesRow = 5;
  ...
  for (let col = 1; col < maxCol; col++) {
    const eventNo = stripLabel(rows[eventRow]?.[col], 'Event');
    const date = stripLabel(rows[dateRow]?.[col], 'Date');
    const course = stripLabel(rows[courseRow]?.[col], 'Course');
    const time = stripLabel(rows[timeRow]?.[col], 'Time');
    const notes = textCell(rows[notesRow]?.[col]);
    const week = textCell(rows[weekRow]?.[col]);

    // Do not show fake Course TBD cards from this tab. If no course/date/event exists, skip it.
    if (!eventNo && !date && !course && !time && !notes) continue;
    if (!course && !eventNo && !date) continue;
