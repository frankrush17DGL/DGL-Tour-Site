...
function StateTrophiesPage({ data, goHome }) {
  ...
  function stateIcon(state) {
    const photo = grouped[state]?.find(record => record.photoUrl)?.photoUrl || '';
    return photo || '';
  }
      ...
      <div className="trophy-map-layout">
        <article className="state-map-card">
          ...
          <div className="usa-map-shape usa-outline-map">
            <svg viewBox="0 0 1000 620" aria-hidden="true" className="usa-svg-map state-outline-svg">
              <path className="usa-land" d="M68,238 L118,192 L205,190 L262,160 L333,178 L400,132 L480,142 L552,125 L635,144 L705,168 L792,202 L870,265 L912,342 L872,410 L785,424 L724,468 L650,464 L588,506 L514,488 L455,523 L384,484 L318,505 L250,462 L182,488 L126,420 Z" />
              <g className="state-lines">
                <path d="M155 203 L145 424" />
                <path d="M225 188 L215 462" />
                <path d="M292 170 L292 492" />
                <path d="M365 160 L363 488" />
                <path d="M435 143 L438 506" />
                <path d="M510 140 L508 492" />
                <path d="M585 132 L580 506" />
                <path d="M660 150 L646 462" />
                <path d="M735 182 L716 465" />
                <path d="M808 220 L780 423" />
                <path d="M112 250 L885 250" />
                <path d="M95 310 L905 310" />
                <path d="M112 370 L880 370" />
                <path d="M150 430 L790 430" />
              </g>
              <path className="state-highlight state-az" d="M225,342 L309,352 L297,448 L232,438 Z" />
              <path className="state-highlight state-fl" d="M738,418 L808,420 L874,496 L858,528 L806,462 L744,454 Z" />
            </svg>
            {stateKeys.map(state => (
              <button
                key={state}
                className={'state-pin trophy-pin photo-trophy-pin ' + (state === selectedState ? 'active' : '')}
                style={statePositions[state] || { left: '50%', top: '50%' }}
                onClick={() => setSelectedState(state)}
              >
                {stateIcon(state) ? <img src={stateIcon(state)} alt={state + ' trophy'} /> : <span>🏆</span>}
          ...
          <div className="state-tabs">
            {stateKeys.map(state => (
              <button key={state} className={state === selectedState ? 'active' : ''} onClick={() => setSelectedState(state)}>
                {stateIcon(state) ? <img src={stateIcon(state)} alt="" className="state-tab-trophy" /> : '🏆'} {state}
              </button>
            ))}
          </div>
        </article>
        ...
        <article className="trophy-feature-card">
          {featuredPhoto ? <img src={featuredPhoto} alt={title} className="trophy-main-photo" /> : <div className="trophy-main-photo placeholder">🏆</div>}
...
function PlayersPage({ data, goHome }) {
  ...
  return (
    <section className="red-room-page players-page">
      <div className="red-page-bg"></div>
      <div className="red-page-vignette"></div>
      <button onClick={goHome} className="back-button">← Back to Home</button>
      ...
      <div className="red-page-hero annals-hero players-hero">
        <div className="red-page-copy">
          ...
          <h1>Player <span>Profiles</span></h1>
          <p>Photos, nicknames, standings, odds, Red Room appearances, State Trophies, and DGLFC history.</p>
          <p className="red-subline">Powered by Players, Annals, Red Rounds, State Trophies, Power Model, and 2026 Standings.</p>
        </div>
      </div>

      <div className="player-card-grid">
        {profiles.length ? profiles.map(profile => (
          <article className="trading-card" key={profile.name}>
            <div className="trading-card-top">
              <PlayerPhoto name={profile.name} src={profile.photoUrl} className="trading-card-photo" />
              <div>
                <span className="card-rank">{profile.rank ? `Rank ${profile.rank}` : 'DGL Member'}</span>
                <h2>{profile.name}</h2>
                <p>{profile.nickname || 'Nickname pending'}</p>
              </div>
            </div>

            <div className="player-stat-grid">
              <div><small>Points</small><strong>{profile.points != null ? profile.points : '—'}</strong></div>
              <div><small>Odds</small><strong>{profile.odds ? formatOdds(profile.odds) : '—'}</strong></div>
              <div><small>DGLFC Apps</small><strong>{profile.dglfcAppearances || 0}</strong></div>
              <div><small>Best DGLFC</small><strong>{profile.bestFinish ? ordinal(profile.bestFinish) : '—'}</strong></div>
              <div><small>Red Room</small><strong>{profile.redRoomCount || 0}</strong></div>
              <div><small>Best Net</small><strong>{profile.bestNet ? formatNet(profile.bestNet) : '—'}</strong></div>
              <div><small>State Wins</small><strong>{profile.stateTrophyWins || 0}</strong></div>
              <div><small>Debut</small><strong>{profile.debutYear || '—'}</strong></div>
            </div>

            {profile.bio ? <p className="player-bio">{profile.bio}</p> : null}
          </article>
        )) : (
