...
function PlayerTradingCard({ profile }) {
  ...
  return (
    <button
      className={'wax80-card ' + (flipped ? 'is-flipped' : '')}
      aria-label={`Flip ${profile.name} trading card`}
      onClick={() => setFlipped(!flipped)}
      style={{ '--card-color': cardColor }}
      type="button"
    >
      <div className="wax80-inner">
        <div className="wax80-face wax80-front">
          <div className="wax80-print">
            <div className="wax80-photo-frame">
              <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="wax80-action-photo" />
              <div className="wax80-logo-badge"><b>DGL</b><span>EST. 2021</span></div>
              <div className="wax80-card-no">#{rankText}</div>
              <div className="wax80-headshot-ring">
                <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="wax80-headshot" />
              </div>
            </div>

            <div className="wax80-name-panel">
              <h2>{profile.name}</h2>
              <p>★ “{title}” ★</p>
            </div>

            <div className="wax80-front-stats">
            ...
            <div className="wax80-ribbon">★ {achievementLine(profile)} ★</div>
            <div className="wax80-league-line">DOJO GOLF LEAGUE</div>
          </div>
        </div>

        <div className="wax80-face wax80-back">
          <div className="wax80-back-print">
            <div className="wax80-back-title">
              <h3>{profile.fullName || profile.name}</h3>
              <div>#{rankText}</div>
              <p>{title}</p>
            </div>

            <div className="wax80-back-grid">
              <section>
