...
const FUTURE_EVENTS_SHEETS = ['Future Events', 'Future events', 'Future Event', 'Upcoming Events'];
...
function parseFutureEvents(text) {
  ...
  const loose = value => normalizeHeader(value);
  const labelMatches = (value, candidates) => {
    const normalized = loose(value);
    return candidates.some(candidate => {
      const normalizedCandidate = loose(candidate);
      return normalized === normalizedCandidate || normalized.includes(normalizedCandidate);
    });
  };

  const findLabel = candidates => {
    for (let row = 0; row < rows.length; row++) {
      for (let col = 0; col < (rows[row] || []).length; col++) {
        if (labelMatches(rows[row][col], candidates)) return { row, col };
      }
    }
    return null;
  };

  // Your Future Events tab is a cross-tab:
  // A1 Week, A2 Event, A3 Date, A4 Course, A5 Time, A6 Notes/commits
  // Events run left-to-right starting in column B.
  const weekPos = findLabel(['Week']) || { row: 0, col: 0 };
  const eventPos = findLabel(['Event']) || { row: 1, col: 0 };
  const datePos = findLabel(['Date']) || { row: 2, col: 0 };
  const coursePos = findLabel(['Course']) || { row: 3, col: 0 };
  const timePos = findLabel(['Time', 'Tee Time', 'Tee Times']) || { row: 4, col: 0 };
  const notesPos = findLabel(['Notes', 'Commit', 'Commits', 'Number of Commits', 'Number of Commits/Notes']) || { row: 5, col: 0 };
  ...
  const maxCol = Math.max(
    rows[weekPos.row]?.length || 0,
    rows[eventPos.row]?.length || 0,
    rows[datePos.row]?.length || 0,
    rows[coursePos.row]?.length || 0,
    rows[timePos.row]?.length || 0,
    rows[notesPos.row]?.length || 0,
  ...
  for (let col = startCol; col < maxCol; col++) {
    const week = textCell(rows[weekPos.row]?.[col]);
    const eventNo = stripLabel(rows[eventPos.row]?.[col], 'Event');
    const date = stripLabel(rows[datePos.row]?.[col], 'Date');
    const course = stripLabel(rows[coursePos.row]?.[col], 'Course');
    const time = stripLabel(rows[timePos.row]?.[col], 'Time');
    const notes = textCell(rows[notesPos.row]?.[col]);

    // Course is the required field. Date/time can be blank without hiding the event.
    if (!course) continue;
