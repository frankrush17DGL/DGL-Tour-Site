...
function PlayerTradingCard({ profile }) {
  ...
  const rankText = profile.rank || String(profile.cardNumber || '').padStart(2, '0');
  ...
  return (
    <article className="gum78-card" style={{ '--card-color': cardColor }}>
      <div className="gum78-stock">
        <div className="gum78-photo">
          <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="gum78-main-photo" />
          <div className="gum78-logo"><b>DGL</b><span>EST. 2021</span></div>
          <div className="gum78-rank"><small>#</small><b>{rankText}</b></div>
        </div>

        <div className="gum78-namebar">
          <h2>{profile.name}</h2>
          <p>★ “{title}” ★</p>
          <div className="gum78-headshot">
            <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="gum78-headshot-photo" />
          </div>
        </div>

        <div className="gum78-stats">
          <div><span>Rank</span><b>{rankText}</b></div>
          <div><span>Points</span><b>{pointsText}</b></div>
          <div><span>Red Rounds</span><b>{redRounds}</b></div>
          <div><span>Best Net</span><b>{bestNetText}</b></div>
        </div>

        <div className="gum78-footer">★ DOJO GOLF LEAGUE ★</div>
      </div>
    </article>
  );
}
