...
function parseFutureEvents(text) {
  ...
  // Future Events is intentionally simple and phone-friendly:
  // Row 1: Week
  // Row 2: Event
  // Row 3: Date
  // Row 4: Course
  ...
  const maxCol = Math.max(
    rows[weekRow]?.length || 0,
    rows[eventRow]?.length || 0,
    rows[dateRow]?.length || 0,
    rows[courseRow]?.length || 0,
    rows[timeRow]?.length || 0,
    rows[notesRow]?.length || 0
  );
  ...
  for (let col = startCol; col < maxCol; col++) {
    ...
    const time = textCell(rows[timeRow]?.[col]);
    const notes = textCell(rows[notesRow]?.[col]);

    if (!course && !eventNo && !date && !time && !notes) continue;
