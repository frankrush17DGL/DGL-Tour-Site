...
function PlayerTradingCard({ profile, cardRef, highlighted = false }) {
  ...
  return (
    <button
      ref={cardRef}
      type="button"
      className={'dgl84-card ' + (flipped ? 'is-flipped ' : '') + edition.className + (highlighted ? ' is-profile-target' : '')}
...
function PlayerCardModal({ profile, onClose }) {
  useEffect(() => {
  ...
  return (
    <div className="dgl-player-modal-backdrop players-page" role="presentation" onMouseDown={onClose}>
      <style>{`
        .dgl-player-modal-backdrop{position:fixed;inset:0;z-index:1400;display:grid;place-items:center;padding:18px;background:rgba(0,0,0,.86);backdrop-filter:blur(7px);overflow:auto}
        .dgl-player-modal{position:relative;width:min(390px,calc(100vw - 36px));margin:auto;padding:48px 10px 18px;border:1px solid rgba(240,199,94,.45);border-radius:24px;background:linear-gradient(180deg,#1b140d,#090706);box-shadow:0 30px 100px rgba(0,0,0,.78)}
        .dgl-player-modal-close{position:absolute;z-index:4;right:12px;top:10px;width:38px;height:38px;border-radius:50%;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.07);color:#fff;font-size:25px;cursor:pointer}
        .dgl-player-modal-label{position:absolute;left:16px;top:17px;margin:0;color:#f0c75e;font-size:9px;font-weight:900;letter-spacing:.14em;text-transform:uppercase}
        .dgl-player-modal-stage{width:min(350px,100%);margin:auto}
        .dgl-player-modal-stage .dgl84-photo-box .player-photo.dgl84-action-photo{width:100%!important;height:100%!important;max-width:none!important;border-radius:0!important;object-fit:cover!important;display:block}
        .dgl-player-modal-stage .dgl84-mini-portrait .player-photo.dgl84-headshot{border-radius:50%!important}
        @media(max-height:740px) and (min-width:600px){.dgl-player-modal{width:330px}.dgl-player-modal-stage{width:290px}}
      `}</style>
      <section className="dgl-player-modal" role="dialog" aria-modal="true" aria-label={`${profile.name} player card`} onMouseDown={event => event.stopPropagation()}>
        <p className="dgl-player-modal-label">Official DGL Player Card • Tap Card to Flip</p>
        <button type="button" className="dgl-player-modal-close" onClick={onClose} aria-label="Close player card">×</button>
        <div className="dgl-player-modal-stage">
          <PlayerTradingCard profile={profile} />
        </div>
      </section>
    </div>
  );
...
function PlayersPage({ data, goHome, selectedPlayer = '' }) {
  const profiles = buildPlayerProfiles(data);
  const selectedCardRef = useRef(null);
  const selectedCanonical = canonicalName(selectedPlayer);

  useEffect(() => {
    if (!selectedCanonical || !selectedCardRef.current) return;
  ...
  }, [selectedCanonical, profiles.length]);

  return (
    <section className="red-room-page players-page card-gallery-page">
      <style>{`
        /*
         * PlayerPhoto has a site-wide circular treatment. The trading card's
         * large action photo is rectangular, while the inset headshot remains
         * circular.
         */
        .players-page .dgl84-photo-box .player-photo.dgl84-action-photo {
          width: 100% !important;
        ...
        .players-page .dgl84-mini-portrait .player-photo.dgl84-headshot {
          border-radius: 50% !important;
        }
        .players-page .dgl84-card.is-profile-target {
          outline: 3px solid #f0c75e;
          outline-offset: 8px;
          box-shadow: 0 0 0 5px rgba(240,199,94,.16), 0 24px 60px rgba(0,0,0,.48);
        }
      `}</style>
      ...
      <div className="dgl84-card-grid">
        {profiles.length ? profiles.map(profile => {
          const highlighted = Boolean(selectedCanonical && canonicalName(profile.name) === selectedCanonical);
          return (
            <PlayerTradingCard
              profile={profile}
              key={profile.name}
              highlighted={highlighted}
              cardRef={highlighted ? selectedCardRef : undefined}
            />
          );
        }) : (
...
export { PlayerTradingCard, PlayerCardModal };
