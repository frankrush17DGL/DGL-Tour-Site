...
function PlayerTradingCard({ profile }) {
  ...
  function achievementLine(item) {
    if (item.bestFinish === 1) return 'DGLFC Champion';
    if (item.stateTrophyWins > 0) return `${item.stateTrophyWins} State Troph${item.stateTrophyWins === 1 ? 'y' : 'ies'}`;
    ...
    return 'Official DGL Player Card';
  }
  ...
  const nameParts = String(profile.name || '').split(' ').filter(Boolean);
  const firstName = nameParts[0] || profile.name;
  const lastName = nameParts.slice(1).join(' ') || '';

  return (
    <button
      className={'flip-card opc-flip-card card31 ' + (flipped ? 'is-flipped' : '')}
      aria-label={`Flip ${profile.name} trading card`}
      onClick={() => setFlipped(!flipped)}
      style={{ '--card-color': cardColor, '--card-accent': accentColor }}
      type="button"
    >
      <div className="flip-card-inner card31-inner">
        <div className="card-face card31-face card31-front">
          <div className="card31-front-inner">
            <div className="card31-topline">
              <span>DOJO GOLF LEAGUE</span>
              <b>2026 • #{cardNo}</b>
            </div>

            <div className="card31-photo-panel">
              <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="card31-action-photo" />
              <div className="card31-photo-shine"></div>
            </div>

            <div className="card31-lower-band">
              <img src="/dgl-logo.jpeg" alt="DGL" className="card31-logo" />
              <div className="card31-headshot-ring">
                <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="card31-headshot" />
              </div>
              <div className="card31-rank-medallion">#{profile.rank || cardNo}</div>

              <div className="card31-nameplate">
                <h2><span>{firstName}</span>{lastName ? <span>{lastName}</span> : null}</h2>
                <p>{title ? `“${title}”` : headlineBadge(profile)}</p>
              </div>
            </div>

            <div className="card31-stat-strip">
              <span><small>Rank</small><b>{profile.rank || '—'}</b></span>
              <span><small>Points</small><b>{profile.points != null ? profile.points : '—'}</b></span>
              <span><small>Trophies</small><b>{profile.stateTrophyWins || 0}</b></span>
            </div>

            <div className="card31-achievement">{achievementLine(profile)}</div>
          </div>
        </div>

        <div className="card-face card31-face card31-back">
          <div className="card31-back-inner">
            <div className="card31-back-title">
              <img src="/dgl-logo.jpeg" alt="DGL" />
              <div>
                <span>DOJO GOLF LEAGUE • CARD #{cardNo}</span>
                <h3>{profile.fullName || profile.name}</h3>
                <p>{title || 'Official Player Card'}</p>
              </div>
            </div>

            <div className="card31-back-id">
              <span>{profile.hometown || 'Hometown TBD'}</span>
              <span>{profile.handedness || 'Hand TBD'}</span>
              <span>Debut {profile.debutYear || '—'}</span>
            </div>

            <div className="card31-back-grid">
              <section>
              ...
              <section>
                ...
                <table>
                  <tbody>
                    ...
                    <tr><th>Job</th><td>{profile.occupation || '—'}</td></tr>
            ...
            <p className="card31-bio"><b>Bio:</b> {profile.bio || 'Bio pending. Add notes in the Players tab to complete the back of this card.'}</p>
            <div className="card31-back-footer"><span>DGL DEBUT: {profile.debutYear || '—'}</span><span>PLAY HARD. GIVE ZERO PUTTS.</span></div>
          </div>
        </div>
      </div>
    </button>
