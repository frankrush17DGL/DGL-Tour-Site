...
const STATE_TROPHY_SHEETS = ['State Trophies'];
...
function parsePlayers(text) {
  ...
  const headerIndex = findHeaderIndex(rows, ['Player']);
  ...
  return rows.slice(headerIndex + 1).map(row => {
    ...
    return {
      ...
      photoUrl: indexes.photoUrl >= 0 ? textCell(row[indexes.photoUrl]) : '',
...
function photoUrlFor(data, name) {
  const url = playerMeta(data, name).photoUrl || '';
  if (!url) return '';
...
function parseFutureEvents(text) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    if (label) labelRows[label] = rowIndex;
  });
  ...
  const timeRow = labelRows.time;
  const teesRow = labelRows.tees;
  const notesRow = labelRows.notes;
  const weekRow = labelRows.week;
  const maxCol = Math.max(...rows.map(row => row.length));
  const events = [];

  for (let col = 1; col < maxCol; col++) {
    const eventNo = textCell(rows[eventRow]?.[col]);
    const date = textCell(rows[dateRow]?.[col]);
    const course = textCell(rows[courseRow]?.[col]);
    ...
    events.push({
      ...
      tees: teesRow !== undefined ? textCell(rows[teesRow]?.[col]) : '',
      time: timeRow !== undefined ? cleanTime(rows[timeRow]?.[col]) : '',
      notes: notesRow !== undefined ? textCell(rows[notesRow]?.[col]) : '',
      week: weekRow !== undefined ? textCell(rows[weekRow]?.[col]) : ''
    });
  }

  return decorateEvents(events, yearFromSheetName(CURRENT_YEAR_SHEET)).filter(event => event.status === 'Upcoming');
}
...
function parseCurrentStandings(sheet) {
  ...
  headerRows.forEach(row => {
    row.forEach((cell, col) => {
      ...
      if (['totaldglpoints', 'totalpoints', 'dglfcpoints', 'currentpts'].includes(h)) totalCol = col;
      if (h === 'rank') rankCol = col;
    });
  });
  ...
  const players = blocks.map(block => {
    ...
    if (pointsRow.length) {
      ...
      if (!points) {
        const numbers = numericValuesFromRow(pointsRow, 4);
        points = Math.round(numbers.reduce((sum, value) => sum + value, 0) * 100) / 100;
      }
    }
  ...
  }).filter(player => player.name && Number.isFinite(player.points));
...
function PlayerPhoto({ name, src = '', className = '' }) {
  ...
  if (failed) {
    return <div className={'player-photo-fallback ' + className}>{initials(name)}</div>;
  }
...
function parseSportsbook(text) {
  ...
  const indexes = {
    player: indexOf(['player', 'name']),
    currentPts: indexOf(['currentpts', 'currentpoints']),
    recentForm: indexOf(['recentform', 'form']),
    ceiling: indexOf(['ceiling', 'ceilingscore', 'redroom', 'performanceindex']),
    attendance: indexOf(['attendance', 'eventplayed', 'eventsplayed']),
    pedigree: indexOf(['pedigree']),
    ghin: indexOf(['currentghin', 'latesthandicap', 'ghin', 'ghinscore']),
    rating: indexOf(['dglrating', 'powerrating', 'powerscore', 'rating']),
    winPercent: indexOf(['win', 'winpercent', 'impliedwin', 'impliedprobability']),
    odds: indexOf(['odds', 'vegasodds']),
    movement: indexOf(['movement', 'move'])
  };

  // Fallback to the standard Power Model layout: A, H, I, J, K.
  ...
  return rows.slice(headerIndex + 1)
    .map(row => {
      ...
      const rating = indexes.rating >= 0 ? numberFromCell(row[indexes.rating]) : 0;
      const winPercent = indexes.winPercent >= 0 ? parsePercentCell(row[indexes.winPercent]) : 0;
      const odds = indexes.odds >= 0 ? String(row[indexes.odds] || '').trim() : '';

      return {
        player,
        currentPts: indexes.currentPts >= 0 ? numberFromCell(row[indexes.currentPts]) : 0,
        recentForm: indexes.recentForm >= 0 ? numberFromCell(row[indexes.recentForm]) : 0,
        ceiling: indexes.ceiling >= 0 ? numberFromCell(row[indexes.ceiling]) : 0,
        attendance: indexes.attendance >= 0 ? numberFromCell(row[indexes.attendance]) : 0,
        pedigree: indexes.pedigree >= 0 ? numberFromCell(row[indexes.pedigree]) : 0,
        ghin: indexes.ghin >= 0 ? numberFromCell(row[indexes.ghin]) : 0,
        rating,
        winPercent,
        odds,
      ...
      <div className="trophy-map-layout">
        <article className="state-map-card">
          ...
          <div className="usa-map-shape">
            <span className="map-label west">WEST</span>
            <span className="map-label midwest">MIDWEST</span>
            {stateKeys.map(state => (
              <button
                key={state}
                className={'state-pin ' + (state === selectedState ? 'active' : '')}
                style={statePositions[state] || { left: '50%', top: '50%' }}
                onClick={() => setSelectedState(state)}
              >
                <span>{stateIcon(state)}</span><b>{state.substring(0, 2).toUpperCase()}</b>
              </button>
            ))}
          </div>
