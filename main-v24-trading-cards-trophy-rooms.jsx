...
function PlayersPage({ data, goHome }) {
  ...
  function headlineBadge(profile) {
    if (profile.stateTrophyWins > 0) return 'TROPHY WINNER';
    ...
    <section className="red-room-page players-page card-gallery-page">
      ...
      <div className="red-page-hero annals-hero players-hero">
        <div className="red-page-copy">
          ...
          <p>Tap a card to flip it. Front is 80s wax-pack glory. Back is career stats.</p>
      ...
      <div className="vintage-card-grid">
        {profiles.length ? profiles.map(profile => (
          <button className="flip-card" key={profile.name} aria-label={`Flip ${profile.name} trading card`}>
            <div className="flip-card-inner">
              <div className="card-face card-front">
                <div className="wax-card-topline">
                  <span>DGL TOUR</span>
                  <b>{headlineBadge(profile)}</b>
                </div>
                <div className="wax-photo-frame">
                  <PlayerPhoto name={profile.name} src={profile.photoUrl} className="wax-player-photo" />
                </div>
                <div className="wax-nameplate">
                  <h2>{profile.name}</h2>
                  <p>{profile.nickname ? `“${profile.nickname}”` : 'Nickname pending'}</p>
                </div>
                <div className="wax-front-stats">
                ...
                <div className="wax-ribbon">
                  {profile.stateTrophyWins > 0 ? `${profile.stateTrophyWins} State Trophy Win${profile.stateTrophyWins === 1 ? '' : 's'}` : profile.redRoomCount > 0 ? `${profile.redRoomCount} Red Room Round${profile.redRoomCount === 1 ? '' : 's'}` : 'Official DGL Player Card'}
                </div>
              </div>

              <div className="card-face card-back">
                <div className="card-back-title">
                  <span>DGL CAREER STATS</span>
                  <h3>{profile.fullName || profile.name}</h3>
                </div>
                <table className="stat-back-table">
                  <tbody>
                ...
                <p className="card-back-bio">{profile.bio || 'Bio pending. Add notes in the Players tab to complete the back of this card.'}</p>
              </div>
            </div>
          </button>
        )) : (
