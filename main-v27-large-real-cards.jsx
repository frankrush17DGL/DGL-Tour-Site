...
function parsePlayers(text) {
  ...
  const indexes = {
    ...
    debutYear: indexOf(['Debut Year', 'Debut']),
    photoUrl: indexOf(['Photo URL', 'Photo', 'Image URL', 'Image', 'Main Photo URL', 'Action Photo URL']),
    headshotUrl: indexOf(['Headshot URL', 'Headshot', 'Portrait URL']),
    bio: indexOf(['Bio', 'Notes']),
    cardColor: indexOf(['Card Color', 'Color', 'Border Color'])
  };
...
function buildPlayerProfiles(data) {
  ...
  return Array.from(names).map(name => {
    ...
    return {
      ...
      headshotUrl: normalizeAssetUrl(meta.headshotUrl || '') || photoUrlFor(data, name),
...
function cardColorTheme(color = '') {
  const raw = String(color || '').trim();
  const key = raw.toLowerCase().replace(/[^a-z0-9]/g, '');

  const themes = {
    blue: ['#1f6fae', '#0b3157', '#f6d365'],
    navy: ['#15396f', '#061b39', '#f6d365'],
    red: ['#b51e2e', '#5b0710', '#f7d06a'],
    crimson: ['#8f0016', '#3f0008', '#f6d365'],
    maroon: ['#7f1722', '#3a080d', '#f7d06a'],
    green: ['#26784b', '#0d321f', '#f4cd66'],
    forestgreen: ['#26784b', '#0d321f', '#f4cd66'],
    orange: ['#c66522', '#5b2608', '#ffe08a'],
    burntorange: ['#c66522', '#5b2608', '#ffe08a'],
    gold: ['#d6a73b', '#7a4c08', '#fff1a8'],
    copper: ['#b66a2c', '#51240c', '#ffe1a1'],
    silver: ['#9ea6ad', '#42484e', '#ffffff'],
    black: ['#202020', '#050505', '#f4d36a'],
    purple: ['#5e397d', '#241032', '#f4d36a'],
    teal: ['#1f7b78', '#0a3433', '#ffe08a']
  };

  const palette = ['blue', 'red', 'green', 'orange', 'navy', 'maroon', 'copper', 'purple', 'teal'];
  const hash = Array.from(raw || 'DGL').reduce((sum, char) => sum + char.charCodeAt(0), 0);
  const picked = themes[key] || themes[palette[hash % palette.length]];

  return {
    '--card-primary': picked[0],
    '--card-secondary': picked[1],
    '--card-accent': picked[2]
  };
}


function CardImage({ name, src = '', className = '', fallbackLabel = '' }) {
  const [failed, setFailed] = useState(false);
  const source = normalizeAssetUrl(src || '');

  if (!source || failed) {
    return (
      <div className={'card-photo-fallback ' + className}>
        <span>🤡</span>
        <b>⛳</b>
        <small>{fallbackLabel || initials(name)}</small>
      </div>
    );
  }

  return <img src={source} alt={name} className={className} onError={() => setFailed(true)} />;
}

function RetroPlayerCard({ profile }) {
  const [flipped, setFlipped] = useState(false);

  function headlineBadge(cardProfile) {
    if (cardProfile.stateTrophyWins > 0) return 'TROPHY WINNER';
    if (cardProfile.bestFinish === 1) return 'DGLFC CHAMPION';
    if (cardProfile.redRoomCount > 0) return 'RED ROOM';
    if (cardProfile.rank) return `RANK ${cardProfile.rank}`;
    return 'DGL TOUR';
  }

  const primaryRibbon = profile.stateTrophyWins > 0
    ? `${profile.stateTrophyWins} STATE TROPHY WIN${profile.stateTrophyWins === 1 ? '' : 'S'}`
    : profile.redRoomCount > 0
      ? `${profile.redRoomCount} RED ROOM ROUND${profile.redRoomCount === 1 ? '' : 'S'}`
      : profile.bestFinish === 1
        ? 'DGLFC CHAMPION'
        : 'OFFICIAL DGL PLAYER CARD';

  return (
    <button
      className={'flip-card retro-card-v2 ' + (flipped ? 'is-flipped' : '')}
      style={cardColorTheme(profile.cardColor || profile.name)}
      onClick={() => setFlipped(!flipped)}
      aria-label={`Flip ${profile.name} trading card`}
    >
      <div className="flip-card-inner">
        <div className="card-face card-front retro-front">
          <div className="retro-card-top">
            <span>DGL TOUR</span>
            <b>{headlineBadge(profile)}</b>
          </div>

          <div className="retro-photo-stack">
            <div className="retro-main-photo">
              <CardImage name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="retro-action-photo" fallbackLabel={initials(profile.name)} />
            </div>
            <div className="retro-headshot-ring">
              <CardImage name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="retro-headshot-photo" fallbackLabel={initials(profile.name)} />
            </div>
            <div className="retro-dgl-logo-badge">
              <img src="/dgl-logo.jpeg" alt="DGL" />
            </div>
          </div>

          <div className="retro-name-bar">
            <span>{profile.debutYear ? `DEBUT ${profile.debutYear}` : 'DGL PLAYER'}</span>
            <h2>{profile.name}</h2>
            <p>{profile.nickname ? `“${profile.nickname}”` : 'Nickname pending'}</p>
          </div>

          <div className="retro-front-stats">
            <span>RANK <b>{profile.rank || '—'}</b></span>
            <span>PTS <b>{profile.points != null ? profile.points : '—'}</b></span>
            <span>ODDS <b>{profile.odds ? formatOdds(profile.odds) : '—'}</b></span>
          </div>

          <div className="retro-bottom-ribbon">{primaryRibbon}</div>
        </div>

        <div className="card-face card-back retro-back">
          <div className="retro-back-header">
            <span>DGL TOUR</span>
            <h3>{profile.fullName || profile.name}</h3>
            <b>Career Statistics</b>
          </div>

          <table className="retro-stat-table">
            <tbody>
              <tr><th>Nickname</th><td>{profile.nickname || '—'}</td></tr>
              <tr><th>Debut</th><td>{profile.debutYear || '—'}</td></tr>
              <tr><th>2026 Rank</th><td>{profile.rank || '—'}</td></tr>
              <tr><th>2026 Points</th><td>{profile.points != null ? profile.points : '—'}</td></tr>
              ...
              <tr><th>Red Room</th><td>{profile.redRoomCount || 0}</td></tr>
              <tr><th>Best Net</th><td>{profile.bestNet ? formatNet(profile.bestNet) : '—'}</td></tr>
              <tr><th>State Wins</th><td>{profile.stateTrophyWins || 0}</td></tr>
              <tr><th>Current Odds</th><td>{profile.odds ? formatOdds(profile.odds) : '—'}</td></tr>
            </tbody>
          </table>

          <div className="retro-bio-box">
            <span>Scouting Report</span>
            <p>{profile.bio || 'Bio pending. Add notes in the Players tab to complete the back of this card.'}</p>
          </div>

          <div className="retro-card-footer">© DGL TOUR • EST. 2021</div>
        </div>
      </div>
    </button>
  );
  ...
  return (
    <section className="red-room-page players-page card-gallery-page">
      ...
      <div className="red-page-hero annals-hero players-hero">
        <div className="red-page-copy">
          <p className="eyebrow">DGL TRADING CARDS</p>
          <h1>Player <span>Cards</span></h1>
          <p>Large retro hockey-card fronts. Stats-only backs. Tap any card to flip.</p>
          <p className="red-subline">One card per row on mobile. Big photo, true back-of-card stats, DGL logo, and color from the Players tab.</p>
        </div>
      </div>

      <div className="vintage-card-grid">
        {profiles.length ? profiles.map(profile => (
          <RetroPlayerCard profile={profile} key={profile.name} />
        )) : (
