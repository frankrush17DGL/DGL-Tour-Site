          ...
          <div className="red-table-full">
            {board.length ? board.map((player, index) => (
              <div className="red-row-full sportsbook-row" key={player.player}>
                ...
                <small>{player.movement || '—'} movement</small>
                <small>{player.valueRating ? Number(player.valueRating).toFixed(2) : '—'} value</small>
                <em>{formatOdds(player.odds)}</em>
                <b>{player.valueTier || '—'}</b>
              </div>
            )) : (
...
function PlayerTradingCard({ profile }) {
  ...
  const pointsText = profile.points != null ? profile.points : '—';
  const bestNetText = profile.bestNet ? formatNet(profile.bestNet) : '—';
  const eventsPlayed = profile.dglfcAppearances || profile.redRoomCount || profile.stateTrophyCount || 0;

  return (
    <button
      className={'dgl70-card dglgum-card ' + (flipped ? 'is-flipped' : '')}
      aria-label={`Flip ${profile.name} trading card`}
      onClick={() => setFlipped(!flipped)}
      style={{ '--card-color': cardColor }}
      type="button"
    >
      <div className="dgl70-inner dglgum-inner">
        <div className="dgl70-face dgl70-front dglgum-front">
          <div className="dgl70-front-stock dglgum-front-stock">
            <div className="dglgum-logo-bug">
              <strong>DGL</strong>
              <span>EST. 2021</span>
            </div>

            <div className="dglgum-rank-badge">
              <span>#</span>
              <b>{rankText}</b>
            </div>

            <div className="dgl70-photo-window dglgum-photo-window">
              <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="dgl70-action-photo dglgum-action-photo" />
              <div className="dgl70-photo-grain dglgum-photo-grain"></div>
            </div>

            <div className="dgl70-headshot-medal dglgum-headshot-medal">
              <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="dgl70-headshot-photo dglgum-headshot-photo" />
            </div>

            <div className="dgl70-name-band dglgum-name-band">
              <div className="dgl70-name-copy dglgum-name-copy">
                <h2>{profile.name}</h2>
                <p><span>★</span> “{title}” <span>★</span></p>
              </div>
            </div>

            <div className="dglgum-front-stats">
              <div><span>Rank</span><b>{rankText}</b></div>
              <div><span>Points</span><b>{pointsText}</b></div>
              <div><span>Red Rounds</span><b>{profile.redRoomCount || 0}</b></div>
              <div><span>Best Net</span><b>{bestNetText}</b></div>
            </div>

            <div className="dglgum-footer-ribbon">
              <span>★</span>
              <b>{achievementLine(profile)}</b>
              <span>★</span>
            </div>
          </div>
        </div>

        <div className="dgl70-face dgl70-back dglgum-back">
          <div className="dgl70-back-stock dglgum-back-stock">
            <div className="dglgum-back-title">
              <h3>{profile.fullName || profile.name}</h3>
              <b>#{rankText}</b>
              <p>{title}</p>
            </div>

            <div className="dglgum-back-statbox">
              <section>
                <h4>DGL Career Stats</h4>
                <table>
                  <tbody>
                    <tr><th>Current Rank</th><td>{rankText}</td></tr>
                    <tr><th>DGL Points</th><td>{pointsText}</td></tr>
                    <tr><th>Events Played</th><td>{eventsPlayed || '—'}</td></tr>
                    ...
                    <tr><th>Best Net Score</th><td>{bestNetText}</td></tr>
                    <tr><th>State Trophies</th><td>{profile.stateTrophyWins || 0}</td></tr>
                    <tr><th>Joined</th><td>{profile.debutYear || '—'}</td></tr>
                  </tbody>
                </table>
              </section>
              ...
              <section>
                ...
                <table>
                  <tbody>
                    ...
                    <tr><th>Throws</th><td>{profile.handedness || '—'}</td></tr>
                    <tr><th>Hometown</th><td>{profile.hometown || '—'}</td></tr>
