...
function parseFutureEvents(text) {
  ...
  const positions = {
    ...
    notes: findPosition([
      ...
      'Number of Competitors',
      'Entry Fee / Number of Competitors'
    ]),
    photo: findPosition([
      'Photo',
      'Photo URL',
    ...
    website: findPosition(['Course Website', 'Website', 'Course URL']),
    map: findPosition(['Google Map', 'Google Maps', 'Map', 'Map URL']),
    courseDetails: findPosition([
      'Yardage / Rating / Slope',
      'Yardage Rating Slope',
      'Course Details',
      'Yardage'
    ]),
    logo: findPosition(['Course Logo', 'Logo', 'Logo URL']),
    scorecard: findPosition(['Scorecard', 'Scorecard URL']),
    flyover: findPosition(['Flyover', 'Flyover URL', 'Video', 'Video URL'])
  };

  if (!positions.event || !positions.date || !positions.course) return [];

  const labelColumns = Object.values(positions).filter(Boolean).map(position => position.col);
  const startCol = Math.max(...labelColumns) + 1;
  ...
  for (let col = startCol; col < maxCol; col++) {
    const week = cellAt(positions.week, col);
    const eventNo = cleanEventNo(cellAt(positions.event, col));
    const date = cleanDate(cellAt(positions.date, col));
    const course = cleanCourse(cellAt(positions.course, col));
    const time = cleanTime(cellAt(positions.time, col));
    const notes = cellAt(positions.notes, col);
    const photoUrl = normalizeAssetUrl(cellAt(positions.photo, col));
    const courseWebsite = cellAt(positions.website, col);
    const googleMap = cellAt(positions.map, col);
    const courseDetails = cellAt(positions.courseDetails, col);
    const courseLogo = normalizeAssetUrl(cellAt(positions.logo, col));
    const scorecardUrl = cellAt(positions.scorecard, col);
    const flyoverUrl = cellAt(positions.flyover, col);

    if (!week && !eventNo && !date && !course && !time && !notes &&
        !photoUrl && !courseWebsite && !googleMap && !courseDetails &&
        !courseLogo && !scorecardUrl && !flyoverUrl) {
      continue;
    }
    ...
    const daysAway = parsed
      ? Math.ceil((parsed.getTime() - today.getTime()) / 86400000)
      : null;

    events.push({
      event: eventNo || String(events.length + 1),
      date,
