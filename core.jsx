...
const CURRENT_YEAR_SHEET_GID = '1446315975';
...
function currentStandingsCsvUrl() {
  return `https://docs.google.com/spreadsheets/d/${SHEET_ID}/export?format=csv&gid=${CURRENT_YEAR_SHEET_GID}&cacheBust=${Date.now()}`;
}
...
function parseFallClassicPot(sheet = []) {
  // 2026 Standings: course names in row 4; Fall Classic pot in row 112.
  // Match the event instead of pinning a column letter. If Gravel Pit appears
  // more than once, the rightmost occurrence is the final regular-season event.
  const courses = sheet[3] || [];
  for (let col = courses.length - 1; col >= 0; col--) {
    if (!/\bgravel\s+pit\b/i.test(String(courses[col] || ''))) continue;
    const amount = strictNumberFromCell(sheet[111]?.[col]);
    return Number.isFinite(amount) && amount >= 0 ? amount : null;
  }
  return null;
}

function parseEventCoordinate(value, maximum) {
...
function parseFutureEvents(text) {
  ...
  if (labelRows.course !== undefined || labelRows.event !== undefined || labelRows.date !== undefined) {
    ...
    for (let col = 1; col < maxCol; col++) {
      ...
      events.push({
        ...
        // Read labels rather than fixed row numbers so sheet edits preserve weather.
        latitude: parseEventCoordinate(valueAt(['Latitude', 'Lat'], col), 90),
        longitude: parseEventCoordinate(valueAt(['Longitude', 'Long', 'Lng', 'Lon'], col), 180),
  ...
  const positions = {
    ...
    latitude: findPosition(['Latitude', 'Lat']),
    longitude: findPosition(['Longitude', 'Long', 'Lng', 'Lon'])
  };
  ...
  for (let col = startCol; col < maxCol; col++) {
    ...
    events.push({
      ...
      latitude: parseEventCoordinate(cellAt(positions.latitude, col), 90),
      longitude: parseEventCoordinate(cellAt(positions.longitude, col), 180),
      notes: '',
      committedPlayers: [],
      tees: '',
      status: parsed && parsed < today ? 'Past' : 'Upcoming',
...
function PlayerLink({ name = '', onSelect, children, className = '' }) {
  const playerName = cleanName(name);
  const label = children ?? playerName;

  if (!playerName || typeof onSelect !== 'function') return <>{label}</>;
...
function parseHistoricalRoundResults(text, sheetName = CURRENT_YEAR_SHEET) {
  ...
  function findLabelAnywhere(labels) {
    const targets = labels.map(cleanLabel);
    for (const target of targets) {
      for (let row = 0; row < sheet.length; row++) {
        const cells = sheet[row] || [];
        for (let col = 0; col < cells.length; col++) {
          if (cleanLabel(cells[col]) === target) return { row, col };
        }
      }
    }
    return null;
  ...
  // The Host row currently sits at row 114, but its position is intentionally
  // discovered by label so the results archive survives future sheet changes.
  const hostPos = findLabelAnywhere(['Host (.05)', 'Host', 'Hosted By']);
  ...
  blocks.forEach(block => {
    ...
    for (let col = startCol; col < maxCol; col++) {
      ...
      results.push({
        ...
        host: hostPos ? cleanName(sheet[hostPos.row]?.[col]) : '',
        player: block.name,
...
function addHistoricalPowerRatings(rounds = [], annalsRecords = []) {
  ...
  [...byYear.keys()].sort((a, b) => a - b).forEach(year => {
    ...
    // Introduce players only when they first appear; future entrants must not
    // change the normalization of an earlier field.
    const state = new Map();

    orderedEvents.forEach(eventRounds => {
      eventRounds.forEach(round => {
        const player = canonicalName(round.player);
        if (!state.has(player)) state.set(player, {
          points: 0, finishes: [], nets: [], attendance: 0, ghin: null,
      ...
      const maxCeiling = Math.max(0, ...states.map(item => item.nets.length ? Math.max(0, 1 - Math.min(...item.nets) / 20) : 0));
      ...
      state.forEach((item, player) => {
        ...
        const recentFormScore = item.finishes.length
          ...
          : 0;
        const ceiling = item.nets.length ? Math.max(0, 1 - Math.min(...item.nets) / 20) : 0;
      ...
      const orderedNets = eventRounds.map(round => Number(round.net)).filter(Number.isFinite).sort((a, b) => a - b);
...
async function loadLiveData() {
  ...
  // The GViz endpoint can return stale/blank formula-backed metadata cells even
  // while the visible sheet is current. The tab-specific export preserves the
  // populated Course row and Host (.05) row used by event history.
  const standingsText = await safeFetchText(currentStandingsCsvUrl(), CURRENT_YEAR_SHEET);
  ...
  const sidePots = safeParse('side pots', fallbackData.sidePots, () => ({
    ...
    fallClassic: parseFallClassicPot(sheet)
  }));
  ...
  const historicalRounds = safeParse('historical round results', fallbackData.historicalRounds, () => {
    ...
    return parsed.length ? addHistoricalPowerRatings(parsed, annalsRecords) : fallbackData.historicalRounds;
  });
  // Keep event consumers backward-compatible with Home.jsx versions that pass
  // only `data.events`. Tournament Center can read the already-loaded history
  // from each event without issuing its own duplicate Google Sheet requests.
  const eventsWithHistory = events.map(event => ({ ...event, historicalRounds }));
