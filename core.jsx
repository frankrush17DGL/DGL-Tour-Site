...
function extractPlayerBlocks(sheet) {
  ...
  for (let row = 0; row < sheet.length; row++) {
    ...
    if (!rawName) continue;
    const rawNameLabel = normalizeHeader(rawName);
    if ([
      'event', 'date', 'course', 'coursename', 'tees', 'time', 'teetime',
      'net', 'ghin', 'rank', 'finish', 'dglfcpoints', 'totaldglpoints',
      'totalpoints', 'seasonpoints'
    ].includes(rawNameLabel)) continue;

    // The standings tabs use a small block for each golfer, but the spacing
    // varies by season. Recognize the block by a nearby NET row plus a nearby
    // points/standing marker instead of requiring DGLFC Points at row + 2.
    const blockRows = sheet.slice(row, Math.min(row + 8, sheet.length));
    const hasNetRow = blockRows.some(blockRow => rowHasLabel(blockRow, 'NET'));
    const hasStandingMarker = blockRows.some(blockRow =>
    ...
    if (!hasNetRow || !hasStandingMarker) continue;
...
function parseHistoricalRoundResults(text, sheetName = CURRENT_YEAR_SHEET) {
  ...
  function findBestLabel(labels, fallbackRow, fallbackCol = 0) {
    ...
    for (let row = 0; row <= Math.min(sheet.length - 1, 14); row++) {
      const cells = sheet[row] || [];
      for (let col = 0; col < cells.length; col++) {
        if (targets.includes(cleanLabel(cells[col]))) {
          // Historical sheets contain repeated labels inside player/stat
          // blocks. The real event-header label is the left-most matching
          // label and has the most populated event cells to its right.
    ...
    orderedEvents.forEach(eventRounds => {
      ...
      state.forEach((item, player) => {
        ...
        const recentFormScore = item.finishes.length
          ? 100 * item.finishes.reduce((sum, value) => sum + value, 0) / item.finishes.length : 0;
        ...
        const ghinScore = Number.isFinite(item.ghin) && maxGhin > minGhin
          ? 100 * (maxGhin - item.ghin) / (maxGhin - minGhin) : 0;
      ...
      const fieldStrength = fieldRatings.length
        ? fieldRatings.reduce((sum, value) => sum + value, 0) / fieldRatings.length : null;
      const fieldSize = eventRounds.length;
      const orderedNets = [...new Set(eventRounds.map(round => Number(round.net)).filter(Number.isFinite))].sort((a, b) => a - b);
...
function parseSportsbook(text) {
  ...
  // Power Model!J2 contains the total sportsbook vig/overround.
  const vig = parsePercentCell(rows[1]?.[9]);
...
async function loadLiveData() {
  ...
  const brianBenJuneSix = historicalRounds.filter(round =>
    round.eventId === '2025-4' &&
    (
      canonicalName(round.player) === canonicalName('Brian Nevala') ||
      canonicalName(round.player) === canonicalName('Ben Magnuson')
    )
  );
  const historyDiagnostics = {
    totalPlayerRounds: historicalRounds.length,
    totalEvents: new Set(historicalRounds.map(round => round.eventId || round.roundId || round.roundKey)).size,
    brianBenJuneSixLoaded:
      brianBenJuneSix.some(round => canonicalName(round.player) === canonicalName('Brian Nevala')) &&
      brianBenJuneSix.some(round => canonicalName(round.player) === canonicalName('Ben Magnuson')),
    brianBenJuneSix
  };

  // Compatibility bridge for Tournament Center implementations that read
  // historical results outside the normal live-data prop handoff.
  if (typeof window !== 'undefined') {
    window.__DGL_HISTORICAL_ROUNDS__ = historicalRounds;
    window.__DGL_HISTORY_DIAGNOSTICS__ = historyDiagnostics;
  }
