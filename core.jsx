...
function parseThisDayHistory(text) {
  ...
  return rows.slice(headerRow + 1).map(row => {
    ...
    return {
      ...
      time,
      event,
      eventNumber: String(event || '').match(/\d+/)?.[0] || '',
      course,
      tees,
      winner,
      second,
  ...
  historyTexts.forEach((text, sheetIndex) => {
    ...
    events.forEach(event => {
      ...
      historyMoments.push({
        ...
        time: event.time || '',
        event: event.event || '',
        eventNumber: String(event.event || '').match(/\d+/)?.[0] || '',
        course: event.course,
        tees: event.tees || '',
        type: '🏌️ Tour Stop',
...
function PlayerLink({ name, onSelect, className = '', children }) {
  const label = children ?? name;
  if (!name || typeof onSelect !== 'function') return <>{label}</>;

  return (
    <button
      type="button"
      className={`dgl-player-name-link ${className}`.trim()}
      onClick={event => {
