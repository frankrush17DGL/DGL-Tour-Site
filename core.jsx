  ...
  events: [
    {
      event: '1',
      date: '8/30/26',
      course: 'Majestic Oaks Signature',
      time: '7:10 AM',
      tees: '',
      committedPlayers: ['Max Olson', 'Keegan Anderson'],
      photoUrl: '/IMG_0155.jpeg',
      courseWebsite: 'https://arcisgolf.com/clubs/majestic_oaks_golf_club/home',
      googleMap: 'https://maps.app.goo.gl/YQ7dBj7skWaC2a4e9?g_st=ic',
      latitude: '45.2222653',
      longitude: '-93.2523319'
    },
    {
      event: '2',
      date: '9/5/26',
      ...
      time: '7:20 AM',
      tees: '',
      committedPlayers: ['Max Olson', 'Joe Grannes', 'John Leitch'],
      photoUrl: '/IMG_0446.jpeg',
      latitude: '45.0323',
      longitude: '-93.2519'
    },
    {
      event: '3',
      date: '9/13/2026',
      course: '4th Annual Family Scramble @ Gem Lake',
      time: '10:00 AM',
      tees: '',
      committedPlayers: ['Frank Rush', 'Brian Nevala', 'Nic Wendel', 'John Leitch', 'Steven Mitchell', 'Alex Rogers', 'Ben Magnuson'],
      photoUrl: '/IMG_0012.jpeg',
      courseWebsite: 'https://gemlakehillsgolf.com',
      googleMap: 'https://maps.apple/p/KYGs7BATggzZb9',
      latitude: '45.0633',
      longitude: '-93.0316'
    },
    {
      ...
      tees: '',
      committedPlayers: [],
      photoUrl: '/fall-moon-golf.jpg',
      courseWebsite: 'https://www.gravelpitgolf.com',
      googleMap: 'https://maps.apple/p/tLaKE~7BSaIIw2',
      latitude: '46.3948',
      longitude: '-94.3435'
    },
    {
      event: '5',
      date: '10/9/2026',
      course: "Cragun's Lehman",
      time: '1:25 PM',
      tees: '',
      committedPlayers: ['Frank Rush', 'Max Olson', 'Brian Nevala', 'Ben Magnuson', 'Scott Wishart', 'Alex Rogers', 'Grant Kleven', 'Kyle Odegaard', 'Tim Perlick', 'AJ Petersen'],
      photoUrl: '/IMG_0064.jpeg',
      latitude: '46.3897',
      longitude: '-94.3708'
    },
    {
      event: '6',
      date: '10/10/2026',
      course: "Classic at Madden's",
      time: '10:40 AM',
      tees: '',
      committedPlayers: ['Frank Rush', 'Max Olson', 'Brian Nevala', 'Ben Magnuson', 'Scott Wishart', 'Alex Rogers', 'Grant Kleven', 'Kyle Odegaard', 'Tim Perlick', 'AJ Petersen'],
      photoUrl: '/IMG_0065.webp',
      latitude: '46.3774',
      longitude: '-94.354'
    },
    {
      event: '7',
      date: '10/11/2026',
      course: "Deacon's Lodge",
      time: '10:50 AM',
      tees: 'Combo',
      committedPlayers: ['Frank Rush', 'Max Olson', 'Brian Nevala', 'Ben Magnuson', 'Scott Wishart', 'Alex Rogers', 'Grant Kleven', 'Kyle Odegaard', 'Tim Perlick', 'AJ Petersen'],
      photoUrl: '/IMG_8445.webp',
      googleMap: 'https://maps.app.goo.gl/ZbtxUR6aMqpBH5nS8?g_st=ic',
      latitude: '46.60535',
      longitude: '-94.20696',
      courseDetails: '6418/71.6/133'
    }
  ],
  redRounds: [
  ...
  if (labelRows.course !== undefined || labelRows.event !== undefined || labelRows.date !== undefined) {
    ...
    for (let col = 1; col < maxCol; col++) {
      ...
      events.push({
        ...
        latitude: valueAt(['Latitude', 'Lat'], col),
        longitude: valueAt(['Longitude', 'Long', 'Lng', 'Lon'], col),
...
function parseHistoricalRoundResults(text, sheetName = CURRENT_YEAR_SHEET) {
  if (!text) return [];
  ...
  function findBestLabel(labels, fallbackRow, fallbackCol = 0) {
    const targets = labels.map(cleanLabel);
    let best = null;

    for (let row = 0; row <= Math.min(sheet.length - 1, 14); row++) {
      const cells = sheet[row] || [];
      for (let col = 0; col < cells.length; col++) {
        if (!targets.includes(cleanLabel(cells[col]))) continue;

        const populatedToRight = cells
          .slice(col + 1)
          .filter(cell => textCell(cell) !== '').length;
        const candidate = { row, col, populatedToRight };

        if (
          !best ||
          candidate.populatedToRight > best.populatedToRight ||
          (
            candidate.populatedToRight === best.populatedToRight &&
  ...
  const eventPos = findBestLabel(['Event', 'Event Number', 'Event #'], 1, 0);
  const datePos = findBestLabel(['Date'], 2, eventPos.col);
  const coursePos = findBestLabel(['Course', 'Course Name'], 3, eventPos.col);
  const teesPos = findBestLabel(['Tees'], 4, eventPos.col);
  const timePos = findBestLabel(['Time', 'Tee Time'], 5, eventPos.col);
  ...
  const maxCol = Math.max(...sheet.map(row => (row || []).length), startCol);
  const blocks = extractPlayerBlocks(sheet);
  const results = [];

  blocks.forEach(block => {
    const netRow = findRowInBlockAny(block, ['NET', 'Net']);
    const ghinRow = findRowInBlockAny(block, ['GHIN', 'Handicap']);
    const pointsRow = findRowInBlockAny(block, ['DGLFC Points', 'Points']);
    if (!netRow.length) return;

    for (let col = startCol; col < maxCol; col++) {
      const net = strictNumberFromCell(netRow[col]);
      if (!Number.isFinite(net)) continue;

      const rawEvent = cleanEventNo(sheet[eventPos.row]?.[col]);
      const rawDate = cleanDate(sheet[datePos.row]?.[col]);
      const course = cleanCourse(sheet[coursePos.row]?.[col]);
      if (!rawEvent && !rawDate && !course) continue;

      const parsedDate = parseLooseDate(rawDate, defaultYear);
      const date = parsedDate
        ? `${parsedDate.getFullYear()}-${String(parsedDate.getMonth() + 1).padStart(2, '0')}-${String(parsedDate.getDate()).padStart(2, '0')}`
        : rawDate;
      const event = rawEvent || `${defaultYear}-${col}`;

      results.push({
        eventId: `${defaultYear}-${event}`,
        roundId: `${defaultYear}-${event}`,
        roundKey: `${defaultYear}-${event}`,
        event,
        eventNumber: event,
        eventName: `Event ${event}`,
        year: defaultYear,
        date,
        dateLabel: rawDate || date,
        course: course || 'Course unavailable',
        tees: cleanTees(sheet[teesPos.row]?.[col]),
        time: cleanTime(sheet[timePos.row]?.[col]),
        player: block.name,
        ...
        score: net,
        netToPar: net,
        ghin: strictNumberFromCell(ghinRow[col]),
        dglPoints: strictNumberFromCell(pointsRow[col])
      });
    }
  });
...
function addHistoricalCourseNames(rounds = [], historyText = '') {
  if (!historyText) return rounds;
  const rows = parseCSV(historyText);
  const headerRow = findHeaderIndex(rows, ['Year', 'Event', 'Course']);
  if (headerRow === -1) return rounds;

  const headers = rows[headerRow].map(normalizeHeader);
  const yearIndex = headerIndex(headers, ['Year']);
  const eventIndex = headerIndex(headers, ['Event', 'Event Number', 'Event #']);
  const courseIndex = headerIndex(headers, ['Course', 'Course Name']);
  if (yearIndex < 0 || eventIndex < 0 || courseIndex < 0) return rounds;

  const courses = new Map();
  rows.slice(headerRow + 1).forEach(row => {
    const year = numberFromCell(row[yearIndex]);
    const event = cleanEventNo(row[eventIndex]);
    const course = cleanCourse(row[courseIndex]);
    if (year && event && course && course !== 'Course TBD') {
      courses.set(`${year}-${event}`, course);
    }
  });

  return rounds.map(round => {
    const year = Number(round.year) || yearFromSheetName(String(round.eventId || ''));
    const event = cleanEventNo(round.eventNumber || round.event);
    const mappedCourse = courses.get(`${year}-${event}`);
    return mappedCourse ? { ...round, course: mappedCourse } : round;
  });
}

// Rebuild the rating each golfer carried immediately before every historical
// event. Tournament Center uses this to rank a future field against prior DGL
// fields on an apples-to-apples, pre-event basis.
function addHistoricalPowerRatings(rounds = [], annalsRecords = []) {
  const byYear = new Map();
  rounds.forEach(round => {
    const year = Number(round.year);
    if (!Number.isFinite(year)) return;
    if (!byYear.has(year)) byYear.set(year, []);
    byYear.get(year).push(round);
  });

  const pedigreeBeforeYear = (player, year) => annalsRecords
    .filter(record => canonicalName(record.player) === player && Number(record.year) < year)
    .reduce((sum, record) => sum + Number(record.weighted || record.pedigree || 0), 0);

  const rated = [];
  [...byYear.keys()].sort((a, b) => a - b).forEach(year => {
    const seasonRounds = byYear.get(year);
    const events = new Map();
    seasonRounds.forEach(round => {
      const key = round.eventId || round.roundId || round.roundKey;
      if (!events.has(key)) events.set(key, []);
      events.get(key).push(round);
    });

    const orderedEvents = [...events.values()].sort((a, b) => {
      const eventA = Number(a[0]?.eventNumber || a[0]?.event || 0);
      const eventB = Number(b[0]?.eventNumber || b[0]?.event || 0);
      if (eventA !== eventB) return eventA - eventB;
      return new Date(a[0]?.date || 0).getTime() - new Date(b[0]?.date || 0).getTime();
    });

    const seasonPlayers = new Set(seasonRounds.map(round => canonicalName(round.player)).filter(Boolean));
    const state = new Map([...seasonPlayers].map(player => [player, {
      points: 0,
      finishes: [],
      nets: [],
      attendance: 0,
      ghin: null,
      pedigree: pedigreeBeforeYear(player, year)
    }]));

    orderedEvents.forEach(eventRounds => {
      eventRounds.forEach(round => {
        const player = canonicalName(round.player);
        const ghin = round.ghin === null || round.ghin === '' ? null : Number(round.ghin);
        if (state.has(player) && Number.isFinite(ghin)) state.get(player).ghin = ghin;
      });

      const states = [...state.values()];
      const maxPoints = Math.max(0, ...states.map(item => item.points));
      const maxCeiling = Math.max(0, ...states.map(item => item.nets.length ? 1 - Math.min(...item.nets) / 20 : 0));
      const maxAttendance = Math.max(0, ...states.map(item => item.attendance));
      const ghins = states.map(item => item.ghin).filter(Number.isFinite);
      const minGhin = ghins.length ? Math.min(...ghins) : null;
      const maxGhin = ghins.length ? Math.max(...ghins) : null;
      const maxPedigree = Math.max(0, ...states.map(item => item.pedigree));
      const ratings = new Map();

      state.forEach((item, player) => {
        const currentPointsScore = maxPoints ? 100 * item.points / maxPoints : 0;
        const recentFormScore = item.finishes.length
          ? 100 * item.finishes.reduce((sum, value) => sum + value, 0) / item.finishes.length
          : 0;
        const ceiling = item.nets.length ? 1 - Math.min(...item.nets) / 20 : 0;
        const ceilingScore = maxCeiling ? 100 * ceiling / maxCeiling : 0;
        const attendanceScore = maxAttendance ? 100 * item.attendance / maxAttendance : 0;
        const ghinScore = Number.isFinite(item.ghin) && Number.isFinite(minGhin) && maxGhin > minGhin
          ? 100 * (maxGhin - item.ghin) / (maxGhin - minGhin)
          : 0;
        const pedigreeScore = maxPedigree ? 100 * item.pedigree / maxPedigree : 0;
        ratings.set(player, Math.round((
          25 * currentPointsScore + 25 * recentFormScore + 15 * ceilingScore +
          5 * attendanceScore + 15 * ghinScore + 15 * pedigreeScore
        ) / 10) / 10);
      });

      const fieldRatings = eventRounds
        .map(round => ratings.get(canonicalName(round.player)))
        .filter(Number.isFinite);
      const fieldStrength = fieldRatings.length
        ? fieldRatings.reduce((sum, value) => sum + value, 0) / fieldRatings.length
        : null;
      const fieldSize = eventRounds.length;
      const orderedNets = [...new Set(
        eventRounds.map(round => Number(round.net)).filter(Number.isFinite)
      )].sort((a, b) => a - b);
      ...
      eventRounds.forEach(round => {
        ...
        rated.push({
          ...round,
          preEventRating: ratings.get(player) ?? null,
          historicalFieldStrength: fieldStrength,
          historicalFieldSize: fieldSize
        });

        const item = state.get(player);
        if (!item) return;
        const net = Number(round.net);
        if (Number.isFinite(net)) {
          const finish = orderedNets.indexOf(net) + 1;
          item.finishes.push(fieldSize > 1 ? (fieldSize - finish) / (fieldSize - 1) : 1);
          item.nets.push(net);
          item.attendance += 1;
        }
        const points = Number(round.dglPoints);
...
function parseSportsbook(text) {
  ...
  // The live vig is stored in the metadata block above the player table
  // (currently Power Model!J1:J2), so capture it before locating Player.
  let vig = 0;
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    const vigColumn = (rows[rowIndex] || []).findIndex(cell => normalizeHeader(cell) === 'vig');
    if (vigColumn === -1) continue;

    for (let valueRow = rowIndex + 1; valueRow < Math.min(rows.length, rowIndex + 4); valueRow++) {
      const parsedVig = parsePercentCell(rows[valueRow]?.[vigColumn]);
      if (Number.isFinite(parsedVig) && parsedVig > 0) {
        vig = Math.min(0.5, Math.max(0, parsedVig));
        break;
      }
    }
    break;
...
async function loadLiveData() {
  ...
  const historicalRounds = safeParse('historical round results', fallbackData.historicalRounds, () => {
    const allTexts = [...historyTexts, standingsText];
    const allSheets = [...HISTORY_SHEETS, CURRENT_YEAR_SHEET];
    const parsed = allTexts.flatMap((text, index) => (
      parseHistoricalRoundResults(text, allSheets[index])
    ));
    const withCourses = addHistoricalCourseNames(parsed, thisDayText);
    return withCourses.length ? addHistoricalPowerRatings(withCourses, annalsRecords) : fallbackData.historicalRounds;
  });
...
export {
  ...
  buildAnnalsYears,
  buildHistoryMoments,
  parseHistoricalRoundResults,
  addHistoricalCourseNames,
  addHistoricalPowerRatings,
