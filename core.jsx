...
const CORE_BUILD = 'CORE 2026.08.03.1';
...
// Google occasionally omits the displayed course values from a full-tab CSV
// export. Requesting sheet row 4 by itself reliably returns those values while
// preserving their original spreadsheet columns.
function historicalCourseRowUrl(sheetName) {
  return `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(sheetName)}&range=A4:ZZ4&cacheBust=${Date.now()}`;
}
...
function parseHistoricalRoundResults(text, sheetName = CURRENT_YEAR_SHEET, courseRowText = '') {
  if (!text) return [];
  ...
  const locateLabels = labels => {
    const targets = labels.map(cleanLabel);
    const matches = [];
    for (let row = 0; row <= Math.min(sheet.length - 1, 18); row++) {
      for (let col = 0; col < (sheet[row] || []).length; col++) {
        if (targets.includes(cleanLabel(sheet[row][col]))) matches.push({ row, col });
      }
    }
    return matches;
  };
  const eventCandidates = locateLabels(['Event', 'Event Number', 'Event #']);
  const metadataMatches = {
    date: locateLabels(['Date']),
    course: locateLabels(['Course', 'Course Name']),
    tees: locateLabels(['Tees']),
    time: locateLabels(['Time', 'Tee Time'])
  };
  const populatedRight = position => (sheet[position.row] || [])
    .slice(position.col + 1)
    .filter(cell => textCell(cell)).length;
  const eventCandidateScore = candidate => {
    const alignedLabels = Object.values(metadataMatches)
      .flat()
      .filter(position => position.col === candidate.col && Math.abs(position.row - candidate.row) <= 8)
      .length;
    return alignedLabels * 1000 + populatedRight(candidate);
  };
  const eventPos = eventCandidates
    .slice()
    .sort((a, b) => eventCandidateScore(b) - eventCandidateScore(a))[0] || { row: 1, col: 0 };
  const nearestAligned = (matches, fallbackRow) => matches
    .filter(position => position.col === eventPos.col && Math.abs(position.row - eventPos.row) <= 8)
    .sort((a, b) => Math.abs(a.row - eventPos.row) - Math.abs(b.row - eventPos.row))[0]
    || { row: fallbackRow, col: eventPos.col };
  const datePos = nearestAligned(metadataMatches.date, 2);
  // Historical standings tabs use a fixed metadata layout: Course is always
  // Google Sheets row 4 (zero-based array index 3). Do not auto-detect this
  // row because repeated "Course" labels elsewhere in the sheet can point
  // the history parser at the wrong section.
  const coursePos = { row: 3, col: eventPos.col };
  const teesPos = nearestAligned(metadataMatches.tees, 4);
  const timePos = nearestAligned(metadataMatches.time, 5);
  const startCol = eventPos.col + 1;
  const maxCol = Math.max(...sheet.map(row => (row || []).length), startCol);
  const supplementalCourseRow = courseRowText
    ? (parseCSV(courseRowText)[0] || [])
    : [];
  const blocks = extractPlayerBlocks(sheet);
  const results = [];
  ...
  blocks.forEach(block => {
    ...
    for (let col = startCol; col < maxCol; col++) {
      ...
      const course = cleanCourse(
        supplementalCourseRow[col] || sheet[coursePos.row]?.[col]
      );
...
async function loadLiveData() {
  ...
  const allHistoricalSheetNames = [...HISTORY_SHEETS, CURRENT_YEAR_SHEET];
  const historicalCourseRowTexts = await Promise.all(
    allHistoricalSheetNames.map(sheet => (
      safeFetchText(historicalCourseRowUrl(sheet), `${sheet} course row`)
    ))
  );
  ...
  const historicalRounds = safeParse('historical round results', fallbackData.historicalRounds, () => {
    ...
    const allSheets = allHistoricalSheetNames;
    const parsed = allTexts.flatMap((text, index) => (
      parseHistoricalRoundResults(text, allSheets[index], historicalCourseRowTexts[index])
    ));
    return parsed.length ? parsed : fallbackData.historicalRounds;
  });
