...
function parseFutureEvents(text) {
  ...
  // Support vertical key/value layout (labels in column A, values in column B).
  const vertical = {};
  for (const row of rows) {
    const key = normalizeHeader(String(row[0] || '').replace(/:/g, ''));
    if (key) vertical[key] = textCell(row[1]);
  }

  if (vertical.course || vertical.event || vertical.date) {
    const parsed = parseLooseDate(vertical.date, new Date().getFullYear());
    const today = new Date();
    today.setHours(0,0,0,0);

    return [{
      week: vertical.week || '',
      event: cleanEventNo(vertical.event || '1'),
      date: cleanDate(vertical.date || ''),
      course: cleanCourse(vertical.course || ''),
      time: cleanTime(vertical.time || ''),
      notes:
        vertical.notesnumberofcommits ||
        vertical.notesnumberofcompetitors ||
        vertical.numberofcompetitors ||
        vertical.numberofcommits ||
        vertical.notes ||
        '',
      photoUrl: normalizeAssetUrl(vertical.photoofcourse || vertical.photo || ''),
      courseWebsite: vertical.coursewebsite || '',
      googleMap: vertical.googlemaps || '',
      courseDetails:
        vertical.yardagesloperating ||
        vertical.yardageratingslope ||
        vertical.coursedetails ||
        '',
      scorecardUrl: vertical.scorecard || '',
      flyoverUrl: vertical.flyover || '',
      courseLogo: normalizeAssetUrl(vertical.courselogo || ''),
      tees: vertical.tees || '',
  ...
  const startCol = Math.max(...Object.values(positions).filter(Boolean).map(p => p.col)) + 1;
  const maxCol = Math.max(...rows.map(r => r.length), startCol);
  ...
  return events.filter(e => e.status !== 'Past').sort((a,b)=>a.timestamp-b.timestamp);
}
