...
function parseFutureEvents(text) {
  ...
  const photoRow = findRow(['Photo', 'Photo URL', 'Course Photo', 'Course Photo URL']);
  ...
  for (let col = 1; col < maxCol; col++) {
    ...
    if (!week && !eventNo && !date && !course && !time && !notes && !photoUrl) continue;
      ...
      <section className="grid">
        ...
        <article className="card events-card tournament-center-v106" id="events">
          ...
          <style>{`
            ...
            @media(max-width:760px){.tournament-center-v106{padding:14px}.tournament-overlay-v106{align-items:start;flex-direction:column}.tournament-facts-v106{grid-template-columns:1fr}.tournament-next-list-v106 .event-list{grid-template-columns:1fr}}
          `}</style>
        </article>
...
function trophyFinishLabel(record, yearRecords = []) {
  ...
  // Some historical rows contain only “#”. Infer the leading tied champions.
  if (raw === '#' || raw === '') {
    const unresolved = yearRecords.filter(item => {
      const value = textCell(item?.finish).toUpperCase().replace(/\s+/g, '');
      return value === '#' || value === '';
    });
    const recordIndex = yearRecords.indexOf(record);
    const firstResolvedIndex = yearRecords.findIndex(item => {
      const value = textCell(item?.finish).toUpperCase().replace(/\s+/g, '');
      return /^T?\d+$/.test(value.replace('#', ''));
    });
    const isLeadingUnresolved = recordIndex >= 0 && (firstResolvedIndex < 0 || recordIndex < firstResolvedIndex);
    if (isLeadingUnresolved && unresolved.length > 1) return 'T1';
    if (isLeadingUnresolved) return '#1';
  }
