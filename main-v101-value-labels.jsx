...
function valueTierLabel(value, player) {
  ...
  const winPct = Number(player?.winPct || player?.win || 0);
  if (winPct <= 0) return '❌ Avoid';
  if (!Number.isFinite(ratio) || ratio <= 0) return '⚖️ Fair';
  if (ratio >= 1.25) return '🟢 Best Value';
  if (ratio >= 1.10) return '✅ Value';
  if (ratio >= 0.90) return '⚖️ Fair';
  if (ratio >= 0.75) return '🔴 Overpriced';
  return '❌ Avoid';
}
