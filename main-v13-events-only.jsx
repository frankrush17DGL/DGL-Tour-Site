...
const FUTURE_EVENTS_SHEETS = ['Future Events', 'Future events', 'Future Event', 'Upcoming Events', 'Event Schedule'];
...
function parsePlayers(text) {
  ...
  const indexes = {
    ...
    photoUrl: indexOf(['Photo URL', 'Photo', 'Image URL', 'Image']),
...
function parseFutureEvents(text) {
  ...
  const rows = rawRows.filter(row => row.some(cell => textCell(cell)));
  if (!rows.length) return [];

  const labelRows = {};
  rows.forEach((row, rowIndex) => {
    const firstCell = textCell(row[0]);
    const normalized = normalizeHeader(firstCell);
    if (!normalized) return;

    // Accept labels like Date, Date:, Course, Course:, Tee, Tees, Time, Tee Time.
    if (normalized === 'week') labelRows.week = rowIndex;
    if (normalized === 'event' || normalized === 'eventnumber' || normalized === 'eventno') labelRows.event = rowIndex;
    if (normalized === 'date') labelRows.date = rowIndex;
    if (normalized === 'course' || normalized === 'coursename') labelRows.course = rowIndex;
    if (normalized === 'tees' || normalized === 'tee') labelRows.tees = rowIndex;
    if (normalized === 'time' || normalized === 'teetime') labelRows.time = rowIndex;
    if (normalized === 'notes' || normalized === 'note') labelRows.notes = rowIndex;
  });

  // If the sheet ever gets changed to row-based headers, support that too.
  const headerRowIndex = rows.findIndex(row =>
    row.some(cell => normalizeHeader(cell) === 'event') &&
    row.some(cell => normalizeHeader(cell) === 'course') &&
    row.some(cell => normalizeHeader(cell) === 'date')
  );

  if (headerRowIndex >= 0 && (labelRows.event === undefined || labelRows.date === undefined || labelRows.course === undefined)) {
    const headers = rows[headerRowIndex].map(normalizeHeader);
    const findIndex = names => headers.findIndex(header => names.includes(header));
    const indexes = {
      week: findIndex(['week']),
      event: findIndex(['event', 'eventnumber', 'eventno']),
      date: findIndex(['date']),
      course: findIndex(['course', 'coursename']),
      tees: findIndex(['tees', 'tee']),
      time: findIndex(['time', 'teetime']),
      notes: findIndex(['notes', 'note'])
    };

    const rowBasedEvents = rows.slice(headerRowIndex + 1).map(row => {
      const eventNo = indexes.event >= 0 ? cleanEventNo(row[indexes.event]) : '';
      const date = indexes.date >= 0 ? cleanDate(row[indexes.date]) : '';
      const course = indexes.course >= 0 ? cleanCourse(row[indexes.course]) : '';
      if (!eventNo && !date && !course) return null;
      return {
        event: eventNo || '',
        date,
        course: course || 'Course TBD',
        tees: indexes.tees >= 0 ? cleanTees(row[indexes.tees]) : '',
        time: indexes.time >= 0 ? cleanTime(row[indexes.time]) : '',
        notes: indexes.notes >= 0 ? textCell(row[indexes.notes]) : '',
        week: indexes.week >= 0 ? textCell(row[indexes.week]) : ''
      };
    }).filter(Boolean);

    return decorateEvents(rowBasedEvents, yearFromSheetName(CURRENT_YEAR_SHEET))
      .filter(event => event.status === 'Upcoming' || !event.timestamp || event.timestamp === Number.MAX_SAFE_INTEGER);
  }

  const eventRow = labelRows.event;
  const dateRow = labelRows.date;
  const courseRow = labelRows.course;

  if (eventRow === undefined || dateRow === undefined || courseRow === undefined) return [];

  const maxCol = Math.max(...rows.map(row => row.length));
  const events = [];

  for (let col = 1; col < maxCol; col++) {
    const eventNo = cleanEventNo(rows[eventRow]?.[col]);
    const date = cleanDate(rows[dateRow]?.[col]);
    const course = cleanCourse(rows[courseRow]?.[col]);
    const time = labelRows.time !== undefined ? cleanTime(rows[labelRows.time]?.[col]) : '';
    const tees = labelRows.tees !== undefined ? cleanTees(rows[labelRows.tees]?.[col]) : '';
    const notes = labelRows.notes !== undefined ? textCell(rows[labelRows.notes]?.[col]) : '';
    const week = labelRows.week !== undefined ? textCell(rows[labelRows.week]?.[col]) : '';

    if (!eventNo && !date && !course && !time) continue;
  ...
  return decorateEvents(events, yearFromSheetName(CURRENT_YEAR_SHEET))
    .filter(event => event.status === 'Upcoming' || !event.timestamp || event.timestamp === Number.MAX_SAFE_INTEGER);
}
...
async function loadLiveData() {
  ...
  const futureEventsText = await fetchFirstAvailableSheet(FUTURE_EVENTS_SHEETS);
  ...
  const futureEvents = parseFutureEvents(futureEventsText);
  const events = futureEvents.length ? futureEvents : parseEventColumns(sheet, CURRENT_YEAR_SHEET);
...
function HomePage({ data, syncStatus, goRedRoom, goAnnals, goStateTrophies, goSportsbook }) {
...
function StateTrophiesPage({ data, goHome }) {
  ...
  function stateIcon(state) {
    if (/florida/i.test(state)) return '🐊';
    if (/arizona/i.test(state)) return '🌵';
    if (/minnesota/i.test(state)) return '❄️';
    if (/wisconsin/i.test(state)) return '🧀';
    return '🏆';
  }

  const statePositions = {
    Florida: { left: '74%', top: '72%' },
    Arizona: { left: '27%', top: '62%' },
    Minnesota: { left: '53%', top: '28%' },
    Wisconsin: { left: '59%', top: '35%' }
  };

  const years = Array.from(new Set(selectedRecords.map(r => r.year))).sort((a, b) => Number(a) - Number(b));

  return (
    <section className="red-room-page trophies-page trophies-v3">
      <div className="red-page-bg"></div>
      <div className="red-page-vignette"></div>
      <button onClick={goHome} className="back-button">← Back to Home</button>
      ...
      <div className="red-page-hero annals-hero">
        <div className="red-page-copy">
          ...
          <p>Click a state. Enter its trophy room.</p>
          <p className="red-subline">Powered by the State Trophies tab.</p>
        </div>
      </div>

      <div className="trophy-map-layout">
        <article className="state-map-card">
          <p className="eyebrow">Interactive Trophy Map</p>
          <div className="usa-map-shape usa-outline-map">
            <svg viewBox="0 0 1000 620" aria-hidden="true" className="usa-svg-map">
              <defs>
                <clipPath id="usaClip">
                  <path d="M80,240 L140,185 L245,198 L305,162 L390,178 L450,125 L560,150 L640,135 L760,175 L870,250 L910,335 L865,410 L760,430 L690,490 L575,470 L500,520 L405,472 L330,505 L260,460 L188,485 L135,410 Z" />
                </clipPath>
              </defs>
              <path className="usa-land" d="M80,240 L140,185 L245,198 L305,162 L390,178 L450,125 L560,150 L640,135 L760,175 L870,250 L910,335 L865,410 L760,430 L690,490 L575,470 L500,520 L405,472 L330,505 L260,460 L188,485 L135,410 Z" />
              <g clipPath="url(#usaClip)" className="state-lines">
                {Array.from({ length: 11 }).map((_, i) => <line key={'v' + i} x1={140 + i * 70} y1="125" x2={140 + i * 70} y2="520" />)}
                {Array.from({ length: 6 }).map((_, i) => <line key={'h' + i} x1="80" y1={170 + i * 58} x2="920" y2={170 + i * 58} />)}
              </g>
            </svg>
            {stateKeys.map(state => (
              <button
                key={state}
                className={'state-pin trophy-pin ' + (state === selectedState ? 'active' : '')}
                style={statePositions[state] || { left: '50%', top: '50%' }}
                onClick={() => setSelectedState(state)}
              >
                <span>🏆</span><b>{state.substring(0, 2).toUpperCase()}</b>
              </button>
            ))}
          </div>
        ...
        <article className="trophy-feature-card">
          {featuredPhoto ? <img src={featuredPhoto} alt={title} className="trophy-main-photo" /> : <div className="trophy-main-photo placeholder">{stateIcon(selectedState)}</div>}
      ...
      <div className="trophy-results-grid">
        {years.map(year => {
          const yearRecords = selectedRecords.filter(record => record.year === year).sort((a, b) => numberFromCell(a.finish) - numberFromCell(b.finish));
          const winner = yearRecords[0];
          return (
            <article className="trophy-year-card" key={selectedState + year}>
              <span>{year}</span>
              <h3>{winner?.course || title}</h3>
              {yearRecords.map(record => (
                <p key={record.player + record.finish}><b>{String(record.finish).startsWith('T') ? record.finish : '#' + record.finish}</b> {record.player}</p>
              ))}
            </article>
          );
        })}
    ...
    <main className="page">
      {page === 'home' && (
        <>
          ...
          <HomePage data={data} syncStatus={syncStatus} goRedRoom={goRedRoom} goAnnals={goAnnals} goStateTrophies={goStateTrophies} goSportsbook={goSportsbook} />
        </>
      )}
