...
const TOURNAMENT_CENTER_BUILD = 'TC 2026.07.31.2';
...
function useHistoricalCourseMetadata(rounds = []) {
  ...
  useEffect(() => {
    ...
    Promise.all(years.map(async year => {
      ...
      // Google GViz can suppress rich/formatted values when rows 1-4 are
      // exported together. Fetching each source row independently returns the
      // displayed values reliably, so preserve their shared column positions.
      ...
      if (!eventResponse.ok || !courseResponse.ok) throw new Error(`Course row request failed for ${year}`);
      ...
      setCourseMap(Object.assign({}, ...maps));
    }).catch(error => {
      if (error.name !== 'AbortError') console.warn('Unable to load historical course row', error);
    });
  ...
  return useMemo(() => rounds.map(round => {
    ...
    const course = courseMap[`${year}-${eventNumber}`];
    return course ? { ...round, course } : round;
  }), [rounds, courseMap]);
}
