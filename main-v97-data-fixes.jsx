...
function parseFutureEvents(text) {
  ...
  const rowHasLabel = (row, labels) => {
    const targets = labels.map(cleanLabel);
    return (row || []).some(cell => targets.includes(cleanLabel(cell)));
  };

  const findRow = labels => {
    const index = rows.findIndex(row => rowHasLabel(row, labels));
  ...
  const hasEventLabel = rows.slice(0, 15).some(row => rowHasLabel(row, ['Event']));
  const hasCourseLabel = rows.slice(0, 15).some(row => rowHasLabel(row, ['Course']));
  const hasDateLabel = rows.slice(0, 15).some(row => rowHasLabel(row, ['Date']));
  if (!hasEventLabel || !hasCourseLabel || !hasDateLabel) {
    return [];
  }
...
function parseStateTrophies(text) {
  ...
  const indexes = {
    ...
    player: headerIndex(headers, ['Player', 'Winner', 'Champion', 'Holder']),
    notes: headerIndex(headers, ['Notes']),
    photoUrl: headerIndex(headers, [
      'Photo URL', 'Photo', 'Image URL', 'Image', 'Picture', 'Picture URL',
      'Trophy Photo', 'Trophy Photo URL', 'Trophy Image', 'Trophy Image URL',
      'Photo Link', 'Trophy Photo Link', 'URL'
    ])
  };

  function imageFromRow(row = []) {
    if (indexes.photoUrl >= 0) {
      const direct = normalizeAssetUrl(row[indexes.photoUrl]);
      if (direct) return direct;
    }

    const imageLike = row.find(cell => {
      const value = textCell(cell);
      return /(^https?:\/\/|^public\/|^\/|\.(png|jpe?g|webp|gif)$)/i.test(value);
    });

    return normalizeAssetUrl(imageLike || '');
  }
...
function parseYearStandings(sheet) {
  ...
  headerRows.forEach(row => {
    row.forEach((cell, col) => {
      const h = normalizeHeader(cell);
      if (['totaldglpoints', 'totalpoints', 'seasonpoints', 'dglpoints', 'points'].includes(h) && col > 0) totalCol = col;
      if (h === 'rank' && col > 0) rankCol = col;
    });
  });

  const blocks = extractPlayerBlocks(sheet);

  function bestPointsFromRow(row = []) {
    const fromTotal = totalCol >= 0 ? numberFromCell(row[totalCol]) : 0;
    if (Number.isFinite(fromTotal) && fromTotal > 0) return fromTotal;

    // Historical sheets have changed shape over time. If the Total Points
    // header is missing/moved, use the largest plausible point value from the
    // points row instead of accidentally using the rank column.
    const nums = row
      .slice(1)
      .map(numberFromCell)
      .filter(value => Number.isFinite(value) && value > 0);
  ...
  return blocks.map(block => {
    const pointsRow = [
      findRowInBlock(block, 'DGLFC Points'),
      findRowInBlock(block, 'DGL Points'),
      findRowInBlock(block, 'Total DGL Points'),
      findRowInBlock(block, 'Points')
    ].find(row => row.length) || [];

    const points = pointsRow.length ? bestPointsFromRow(pointsRow) : 0;
    const rank = pointsRow.length && rankCol >= 0 ? numberFromCell(pointsRow[rankCol]) : 999;
  ...
  }).filter(player => player.name && (player.points > 0 || player.rank < 999))
    .sort((a, b) => {
      // If the annual sheet has an explicit rank, use it. That keeps the
      // Annals aligned with the official season standings even when old sheet
      // layouts move the point-total column.
      if (a.rank < 999 || b.rank < 999) return a.rank - b.rank || b.points - a.points || a.name.localeCompare(b.name);
      return b.points - a.points || a.name.localeCompare(b.name);
    });
}
...
function buildAnnalsYearsFromRecords(records = [], historyTexts = []) {
  ...
  return Array.from(years).map(year => {
    ...
    const topPoints = standings.find(player => Number(player.rank) === 1) || standings[0] || {};
