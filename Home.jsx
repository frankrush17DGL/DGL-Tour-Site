...
function HomePage({ data, syncStatus, goRedRoom, goAnnals, goStateTrophies, goSportsbook, goPlayers, goRules, goPlayerProfile }) {
  data = {
    ...fallbackData,
    ...(data || {}),
    ...
    <>
      <section className="hero">
        ...
        <div className="hero-stats">
          <div className="hero-stat leader">
            ...
            <strong><PlayerLink name={leader.name} onSelect={goPlayerProfile} /></strong>
          ...
          <div className="hero-stat">
            ...
            <strong><PlayerLink name={featuredRedRound.player} onSelect={goPlayerProfile}>{featuredRedRound.player || 'Red Room'}</PlayerLink></strong>
      ...
      <section className="grid">
          ...
          <div className="podium pro-podium">
            {top10.slice(0, 3).map(player => (
              <div className={'podium-card rank-' + player.rank} key={player.name}>
                ...
                <strong><PlayerLink name={player.name} onSelect={goPlayerProfile} /></strong>
          ...
          <div className="table">
            {top10.map(player => (
              <div className={'row ' + (player.rank <= 3 ? 'top-row' : '')} key={player.name}>
                ...
                <span className="row-player"><PlayerPhoto name={player.name} src={photoUrlFor(data, player.name)} /><PlayerLink name={player.name} onSelect={goPlayerProfile} /></span>
                <em>{player.points} pts</em>
              </div>
            ))}
          </div>
        ...
        <article className="card sportsbook-card" id="sportsbook">
          ...
          <p><strong>Favorite:</strong> <PlayerLink name={insights.favorite.player || leader.name} onSelect={goPlayerProfile} /> {insights.favorite.odds ? formatOdds(insights.favorite.odds) : ''}</p>
          <p><strong>Best Value:</strong> <PlayerLink name={insights.bestValue.player} onSelect={goPlayerProfile}>{insights.bestValue.player || 'Model loading'}</PlayerLink></p>
          <p><strong>Highest Ceiling:</strong> <PlayerLink name={insights.highestCeiling.player} onSelect={goPlayerProfile}>{insights.highestCeiling.player || 'Model loading'}</PlayerLink></p>
