...
function HomePage({ data, syncStatus, goRedRoom, goAnnals, goStateTrophies, goSportsbook, goPlayers, goPlayerProfile, goRules }) {
  const suppliedEvents = Array.isArray(data?.events) ? data.events : [];
  const [recoveredEvents, setRecoveredEvents] = useState([]);

  useEffect(() => {
    if (suppliedEvents.length) return undefined;

    let active = true;

    fetchFutureEventsSheet()
      .then(text => parseFutureEvents(text))
      .then(events => {
        if (active && Array.isArray(events) && events.length) {
          setRecoveredEvents(events);
        }
      })
      .catch(error => {
        console.warn('Homepage unable to recover Future Events', error);
      });
  ...
  data = {
    ...
    events: suppliedEvents.length
      ? suppliedEvents
      : (recoveredEvents.length
        ? recoveredEvents
        : fallbackData.events.filter(event => {
    ...
    <>
      <section className="hero">
        ...
        <div className="hero-stats">
          <div className="hero-stat leader">
            ...
            <strong>{leader.name ? <button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(leader.name)}>{leader.name}</button> : ''}</strong>
          ...
          <div className="hero-stat">
            ...
            <strong>{featuredRedRound.player ? <button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(featuredRedRound.player)}>{featuredRedRound.player}</button> : 'Red Room'}</strong>
      ...
      <section className="grid">
          ...
          <div className="podium pro-podium">
            {top10.slice(0, 3).map(player => (
              <div className={'podium-card rank-' + player.rank} key={player.name}>
                ...
                <strong><button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(player.name)}>{player.name}</button></strong>
          ...
          <div className="table">
            {top10.map(player => (
              <div className={'row ' + (player.rank <= 3 ? 'top-row' : '')} key={player.name}>
                ...
                <span className="row-player"><PlayerPhoto name={player.name} src={photoUrlFor(data, player.name)} /><button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(player.name)}>{player.name}</button></span>
                <em>{player.points} pts</em>
              </div>
            ))}
          </div>
        ...
        <TournamentCenter
          ...
          rounds={data.historicalRounds || data.rounds || data.roundResults || []}
          leader={leader}
        ...
        <article className="card sportsbook-card" id="sportsbook">
          ...
          <p><strong>Favorite:</strong> {(insights.favorite.player || leader.name) ? <button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(insights.favorite.player || leader.name)}>{insights.favorite.player || leader.name}</button> : 'Model loading'} {insights.favorite.odds ? formatOdds(insights.favorite.odds) : ''}</p>
          <p><strong>Best Value:</strong> {insights.bestValue.player ? <button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(insights.bestValue.player)}>{insights.bestValue.player}</button> : 'Model loading'}</p>
          <p><strong>Highest Ceiling:</strong> {insights.highestCeiling.player ? <button type="button" className="dgl-player-link" onClick={() => goPlayerProfile?.(insights.highestCeiling.player)}>{insights.highestCeiling.player}</button> : 'Model loading'}</p>
