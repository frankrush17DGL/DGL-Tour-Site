...
function valueTierFromRating(value) {
...
function parseSportsbook(text) {
  ...
  const indexes = {
    ...
    lastWeekOdds: indexOf(['Last Week Odds', 'Last week odds', 'Previous Odds']),
  ...
  return rows.slice(headerIndex + 1)
    .map(row => {
      ...
      const valueRating = numberFromCell(row[indexes.valueRating]);
      ...
      return {
        ...
        valueRating,
        valueTier: valueTierFromRating(valueRating)
      };
    })
    .filter(Boolean)
        ...
        <article className="card sidepots-card">
          ...
          <div className="pot-winner-scroll" aria-label="Past Eagle Pot winners">
            <div className="pot-winner-track">
              ...
              <span><strong>Prior Eagle Pot</strong> Scott Wishart • Edinburgh Golf Course • Hole #1 • 492-yard Par 5 • 6/18/26 • Paid $77.50</span>
          ...
          <div className="red-table-full">
            {board.length ? board.map((player, index) => (
              <div className="red-row-full sportsbook-row" key={player.player}>
                ...
                <b>{player.movement || '—'}</b>
                <i>{player.valueTier || valueTierFromRating(player.valueRating)}</i>
              </div>
            )) : (
...
function PlayerTradingCard({ profile }) {
  ...
  const rankLabel = profile.rank || cardNo;

  return (
    <button
      className={'gum-card ' + (flipped ? 'is-flipped' : '')}
      aria-label={`Flip ${profile.name} trading card`}
      onClick={() => setFlipped(!flipped)}
      style={{ '--gum-color': cardColorValue(profile.cardColor) }}
      type="button"
    >
      <div className="gum-card-inner">
        <article className="gum-side gum-front">
          <div className="gum-border">
            <div className="gum-photo-wrap">
              <div className="gum-logo-box"><strong>DGL</strong><span>EST. 2021</span></div>
              <div className="gum-rank-circle"><small>#</small><b>{rankLabel}</b></div>
              <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="gum-main-photo" />
              <div className="gum-headshot-ring">
                <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="gum-headshot" />
              </div>
            </div>

            <div className="gum-nameplate">
              <h2>{profile.name}</h2>
              <p>★ “{title}” ★</p>
            </div>

            <div className="gum-front-stats">
            ...
            <div className="gum-footer-ribbon">★ {achievementLine(profile)} ★</div>
          </div>
        </article>

        <article className="gum-side gum-back">
          <div className="gum-border gum-back-border">
            <div className="gum-back-title">
              <h2>{profile.fullName || profile.name}</h2>
              <div className="gum-rank-circle back-rank"><small>#</small><b>{rankLabel}</b></div>
            </div>
            <p className="gum-subtitle">{title}</p>

            <div className="gum-back-grid">
              <section>
                <h3>DGL Career Stats</h3>
                <table><tbody>
                  <tr><th>Current Rank</th><td>{profile.rank || '—'}</td></tr>
                  <tr><th>DGL Points</th><td>{profile.points != null ? profile.points : '—'}</td></tr>
            ...
            <div className="gum-bio">
              <h3>★ Player Bio ★</h3>
              <p>{profile.bio || 'Bio pending. Add notes in the Players tab to complete the back of this card.'}</p>
            </div>

            <div className="gum-back-footer"><span>DOJO GOLF LEAGUE</span><b>EST. 2021</b></div>
          </div>
        </article>
      </div>
    </button>
