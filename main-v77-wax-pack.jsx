...
function PlayerTradingCard({ profile }) {
  ...
  const eventWins = profile.bestFinish === 1 ? 1 : (profile.stateTrophyWins || 0);
  const redRounds = profile.redRoomCount || 0;
  const joined = profile.debutYear || '2021';

  return (
    <article className="wax77-card" style={{ '--card-color': cardColor }}>
      <div className="wax77-cardstock">
        <div className="wax77-print-wear"></div>

        <div className="wax77-photo-stage">
          <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="wax77-main-photo" />
          <div className="wax77-logo-box">
            <strong>DGL</strong>
            <span>EST. 2021</span>
          </div>
          <div className="wax77-number"><span>#</span><b>{rankText}</b></div>
        </div>

        <div className="wax77-nameplate">
          <h2>{profile.name}</h2>
          <p>★ “{title}” ★</p>
          <div className="wax77-portrait">
            <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="wax77-headshot" />
          </div>
        </div>

        <div className="wax77-stat-card">
          <div className="wax77-stat-title">DGL Career Stats</div>
          <div className="wax77-stat-title">Player Facts</div>
          <dl><dt>Current Rank</dt><dd>{rankText}</dd></dl>
          <dl><dt>DGL Points</dt><dd>{pointsText}</dd></dl>
          <dl><dt>Event Wins</dt><dd>{eventWins}</dd></dl>
          <dl><dt>Red Rounds</dt><dd>{redRounds}</dd></dl>
          <dl><dt>Best Net</dt><dd>{bestNetText}</dd></dl>
          <dl><dt>Joined</dt><dd>{joined}</dd></dl>
        </div>

        <div className="wax77-bio-strip">
          <span>★</span>
          <b>{profile.bio || 'Dojo Golf League player card'}</b>
          <span>★</span>
        </div>

        <div className="wax77-footer">DOJO GOLF LEAGUE</div>
      </div>
    </article>
  );
}
