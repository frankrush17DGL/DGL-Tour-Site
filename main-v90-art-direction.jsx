...
function valueTierLabel(value) {
  ...
  if (!Number.isFinite(n) || n <= 0) return '⚖️ Fair';
  if (n >= 1.20) return '🔥 Strong Value';
  if (n >= 1.05) return '✅ Value';
  if (n >= 0.95) return '⚖️ Fair';
  return '❌ Overpriced';
}
...
function valueTierClass(value) {
  ...
  if (n >= 1.20) return 'strong';
  if (n >= 1.05) return 'value';
  if (n >= 0.95) return 'fair';
  return 'over';
}
      ...
      <article className="dgl84-odds-board">
        ...
        <div className="dgl84-odds-table">
          ...
          {board.length ? board.map((player, index) => {
            ...
            const tier = valueTierLabel(player.valueRating);
            const tierClass = valueTierClass(player.valueRating);
            return (
...
function PlayerTradingCard({ profile }) {
  ...
  const [first, last] = splitCardName(profile.name);
  const edition = cardEdition(profile);
  const title = profile.playerTitle || profile.nickname || edition.label;
  const cardNo = String(profile.cardNumber || '').replace(/^0+/, '') || String(profile.rank || '1').replace(/^#/, '');
  ...
  return (
    <button
      type="button"
      className={'dgl90-card ' + (flipped ? 'is-flipped ' : '') + edition.className}
      onClick={() => setFlipped(value => !value)}
      aria-label={`Flip ${profile.name} official DGL card`}
      style={{ '--card-color': cardColor, '--card-accent': accent }}
    >
      <div className="dgl90-rotor">
        <div className="dgl90-face dgl90-front">
          <div className="dgl90-front-stock">
            <div className="dgl90-topline"><span>DOJO GOLF LEAGUE</span><b>{edition.label}</b></div>

            <div className="dgl90-photo-frame">
              <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="dgl90-action-photo" />
              <div className="dgl90-photo-grain" />
              <div className="dgl90-logo-stamp"><strong>DGL</strong><span>EST. 2021</span></div>
              <div className="dgl90-number-medallion">{rankDisplay}</div>
            </div>

            <div className="dgl90-nameplate">
              <div className="dgl90-name-copy">
                <h2><span>{first}</span>{last ? <span>{last}</span> : null}</h2>
                <p>★ {String(title || 'DGL TOUR').toUpperCase()} ★</p>
              </div>
            </div>

            <div className="dgl90-headshot-ring">
              <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="dgl90-headshot" />
            </div>

            <div className="dgl90-front-footer"><span>OFFICIAL DGL CARD</span><span>CARD #{cardNo}</span></div>
          </div>
        </div>

        <div className="dgl90-face dgl90-back">
          <div className="dgl90-back-stock">
            <div className="dgl90-back-header">
              <div className="dgl90-back-logo"><strong>DGL</strong><span>2026</span></div>
              <div><p>Official Player Record</p><h3>{profile.fullName || profile.name}</h3><span>{title} • Card #{cardNo}</span></div>
              <b>{rankDisplay}</b>
            </div>

            <div className="dgl90-back-idline"><span>{profile.hometown || 'MPLS, MN'}</span><span>{profile.handedness || 'RIGHT'}</span><span>{edition.label}</span></div>

            <div className="dgl90-back-columns">
              <section><h4>Career Stats</h4><dl>
            ...
            <div className="dgl90-scouting"><h4>Scouting Report</h4><p>{profile.bio || 'Bio pending. Add notes in the Players tab to complete the back of this card.'}</p></div>
            <div className="dgl90-achievements"><span>{profile.bestFinish === 1 ? 'Career Achievement: DGLFC Champion Series' : profile.redRoomCount > 0 ? 'Career Achievement: Red Room Round' : 'Career Achievement: Official Tour Member'}</span></div>
            <div className="dgl90-back-foot"><span>Dojo Golf League</span><span>Est. 2021</span></div>
          </div>
        </div>
      </div>
    </button>
