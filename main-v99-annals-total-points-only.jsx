...
function valueImmediatelyAfterLabel(row = [], labels = []) {
  const normalizedLabels = labels.map(normalizeHeader);
  for (let col = 0; col < row.length; col++) {
    if (!normalizedLabels.includes(normalizeHeader(row[col]))) continue;
    for (let nextCol = col + 1; nextCol < row.length; nextCol++) {
      const value = numberFromCell(row[nextCol]);
...
function findExactHeaderColumn(sheet, labels = []) {
  const normalizedLabels = labels.map(normalizeHeader);
  let found = -1;

  sheet.slice(0, 25).forEach(row => {
    row.forEach((cell, col) => {
      if (normalizedLabels.includes(normalizeHeader(cell))) {
        found = col;
      }
    });
  });
...
function parseYearStandings(sheet) {
  // Important: this intentionally ignores generic "Points" and never falls back
  // to the Rank column. Every historical standings sheet uses "Total DGL Points".
  const totalCol = findExactHeaderColumn(sheet, ['Total DGL Points']);
  const rankCol = findExactHeaderColumn(sheet, ['Rank']);

  const blocks = extractPlayerBlocks(sheet);
  const players = blocks.map(block => {
    const pointsRow = findRowInBlock(block, 'Total DGL Points');
    const rankRow = findRowInBlock(block, 'Rank');

    let points = 0;
    if (pointsRow.length) {
      const direct = totalCol >= 0 ? numberFromCell(pointsRow[totalCol]) : NaN;
      points = Number.isFinite(direct) && direct !== 0
        ? direct
        : valueImmediatelyAfterLabel(pointsRow, ['Total DGL Points']);
    }

    let rank = 999;
    if (rankRow.length) {
      const directRank = rankCol >= 0 ? numberFromCell(rankRow[rankCol]) : NaN;
      const fallbackRank = valueImmediatelyAfterLabel(rankRow, ['Rank']);
      rank = Number.isFinite(directRank) && directRank !== 0 ? directRank : (fallbackRank || 999);
    }
  ...
  }).filter(player => player.name && Number.isFinite(player.points) && player.points > 0);
  ...
  return players.sort((a, b) =>
    ...
    ...player,
    rank: player.rank && player.rank !== 999 ? player.rank : index + 1
  }));
}
