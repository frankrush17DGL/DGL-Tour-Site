...
function parsePlayers(text) {
  ...
  const headerIndex = rows.findIndex(row =>
    row.some(cell => ['player', 'name'].includes(normalizeHeader(cell)))
  );
  ...
  const indexes = {
    ...
    nickname: indexOf(['Nickname']),
    playerTitle: indexOf(['Player Title', 'Title', 'Card Title']),
    fullName: indexOf(['Full Name', 'FullName']),
    dob: indexOf(['DOB', 'Date of Birth', 'Birthdate']),
    debutYear: indexOf(['Debut Year', 'Debut', 'DGL Debut']),
    height: indexOf(['Height']),
    handedness: indexOf(['Handedness', 'Hand', 'Throws', 'Bats Throws']),
    hometown: indexOf(['Hometown', 'Home Town']),
    occupation: indexOf(['Occupation', 'Job']),
    favoriteCourse: indexOf(['Favorite Course', 'Favorite Track', 'Home Course']),
    walkUpSong: indexOf(['Walk-Up Song', 'Walk Up Song', 'Walkup Song']),
    photoUrl: indexOf(['Photo URL', 'Photo', 'Image URL', 'Image', 'Action Photo URL']),
    headshotUrl: indexOf(['Headshot URL', 'Headshot', 'Head Shot URL', 'Portrait URL']),
    cardColor: indexOf(['Card Color', 'Color', 'Team Color']),
    playerNumber: indexOf(['Player Number', 'Card Number', 'Number']),
    bio: indexOf(['Bio', 'Notes'])
  };

  return rows.slice(headerIndex + 1).map(row => {
    const name = cleanName(row[indexes.player]);
    if (!name) return null;
    return {
      name,
      fullName: indexes.fullName >= 0 ? textCell(row[indexes.fullName]) : name,
      nickname: indexes.nickname >= 0 ? textCell(row[indexes.nickname]) : '',
      playerTitle: indexes.playerTitle >= 0 ? textCell(row[indexes.playerTitle]) : '',
      dob: indexes.dob >= 0 ? textCell(row[indexes.dob]) : '',
      debutYear: indexes.debutYear >= 0 ? textCell(row[indexes.debutYear]) : '',
      height: indexes.height >= 0 ? textCell(row[indexes.height]) : '',
      handedness: indexes.handedness >= 0 ? textCell(row[indexes.handedness]) : '',
      hometown: indexes.hometown >= 0 ? textCell(row[indexes.hometown]) : '',
      occupation: indexes.occupation >= 0 ? textCell(row[indexes.occupation]) : '',
      favoriteCourse: indexes.favoriteCourse >= 0 ? textCell(row[indexes.favoriteCourse]) : '',
      walkUpSong: indexes.walkUpSong >= 0 ? textCell(row[indexes.walkUpSong]) : '',
      photoUrl: indexes.photoUrl >= 0 ? normalizeAssetUrl(row[indexes.photoUrl]) : '',
      headshotUrl: indexes.headshotUrl >= 0 ? normalizeAssetUrl(row[indexes.headshotUrl]) : '',
      cardColor: indexes.cardColor >= 0 ? textCell(row[indexes.cardColor]) : '',
      playerNumber: indexes.playerNumber >= 0 ? textCell(row[indexes.playerNumber]) : '',
      bio: indexes.bio >= 0 ? textCell(row[indexes.bio]) : ''
    };
  }).filter(Boolean);
}
function playerMeta(data, name) {
  const key = canonicalName(name);
  return (data.players || []).find(player => canonicalName(player.name) === key) || {};
}

function normalizeAssetUrl(url = '') {
  const raw = textCell(url);
  if (!raw) return '';
...
function parseThisDayHistory(text) {
  if (!text) return [];
  const rows = parseCSV(text);
  const headerIndex = findHeaderIndex(rows, ['Date']);
  if (headerIndex === -1) return [];
  ...
  const indexes = {
    ...
    photoUrl: indexOf(['Photo URL', 'Photo', 'Image URL'])
  };

  return rows.slice(headerIndex + 1).map(row => {
    const rawDate = textCell(row[indexes.date]);
    const year = indexes.year >= 0 ? textCell(row[indexes.year]) : '';
    if (!rawDate) return null;
    const date = monthDayFromDate(rawDate, numberFromCell(year) || new Date().getFullYear());
    if (!date) return null;
    ...
    const title = indexes.title >= 0 ? textCell(row[indexes.title]) : '';
    const story = indexes.story >= 0 ? textCell(row[indexes.story]) : '';
    const fallbackStory = [
      winner ? `${winner} took first${net ? ` with a ${net} net` : ''}.` : '',
    ...
    return {
      ...
      title: title || (winner ? `${winner} enters the archive` : 'DGL history recorded'),
      body: story || fallbackStory,
      photoUrl: indexes.photoUrl >= 0 ? normalizeAssetUrl(row[indexes.photoUrl]) : ''
    };
  }).filter(Boolean);
}
...
function parseStateTrophies(text) {
  if (!text) return [];
  const rows = parseCSV(text);
  const headerIndex = findHeaderIndex(rows, ['State', 'Year', 'Player']);
  if (headerIndex === -1) return [];
  ...
  const indexes = {
    state: indexOf(['State']),
    trophy: indexOf(['Trophy Name', 'Trophy']),
    year: indexOf(['Year']),
    course: indexOf(['Course']),
    finish: indexOf(['Finish', 'Place']),
    player: indexOf(['Player', 'Winner']),
    notes: indexOf(['Notes']),
    photoUrl: indexOf(['Photo URL', 'Photo', 'Image URL'])
  };
...
function PlayerPhoto({ name, src = '', className = '' }) {
  ...
  const source = src || `/players/${slug}.jpg`;
...
async function loadLiveData() {
  ...
  const sportsbookText = await fetchFirstAvailableSheet(SPORTSBOOK_SHEETS);
  const annalsText = await fetchFirstAvailableSheet(ANNALS_SHEETS);
  const playersText = await fetchFirstAvailableSheet(PLAYERS_SHEETS);
  const stateTrophiesText = await fetchFirstAvailableSheet(STATE_TROPHY_SHEETS);
  const thisDayText = await fetchFirstAvailableSheet(THIS_DAY_SHEETS);
  const futureEventsText = await fetchFutureEventsSheet();

  const sheet = parseCSV(standingsText);
  const redSheet = parseCSV(redRoundsText);

  const standings = parseCurrentStandings(sheet);
  ...
  const historyMoments = directHistoryMoments.length ? directHistoryMoments : buildHistoryMoments(historyTexts);
  const annalsRecords = parseAnnalsRecords(annalsText);
  const annalsYears = annalsRecords.length ? buildAnnalsYearsFromRecords(annalsRecords, historyTexts) : buildAnnalsYears(historyTexts);
  const sportsbook = parseSportsbook(sportsbookText);
  const players = parsePlayers(playersText);
  const stateTrophies = parseStateTrophies(stateTrophiesText);
    ...
    <section className="red-room-page sportsbook-page">
      ...
      <div className="red-page-hero annals-hero">
        <div className="red-page-copy">
          ...
          <p className="red-subline">DGL Rating • Implied Probability • Vegas Odds • Line Movement</p>
        </div>
      </div>
...
function StateTrophiesPage({ data, goHome }) {
  ...
  const [selectedState, setSelectedState] = useState(stateKeys[0] || 'Florida');
  const selectedRecords = grouped[selectedState] || [];
  const title = selectedRecords[0]?.trophy || selectedState;
  const featuredPhoto = selectedRecords.find(record => record.photoUrl)?.photoUrl || '';
  ...
  useEffect(() => {
    if (stateKeys.length && !stateKeys.includes(selectedState)) setSelectedState(stateKeys[0]);
  }, [stateKeys.join('|'), selectedState]);

  function trophyPhoto(state) {
    return grouped[state]?.find(record => record.photoUrl)?.photoUrl || '';
  }
        ...
        {stateKeys.map(state => {
          ...
          return (
            <button key={state} className={state === selectedState ? 'active' : ''} onClick={() => setSelectedState(state)}>
              {photo ? <img src={photo} alt={state + ' trophy'} /> : <span>🏆</span>}
              <b>{state}</b>
      ...
      <div className="trophy-room-layout">
        <article className="trophy-hero-card">
          <div className="trophy-hero-photo-wrap">
            {featuredPhoto ? <img src={featuredPhoto} alt={title} className="trophy-hero-photo" /> : <div className="trophy-hero-photo placeholder">🏆</div>}
          </div>
...
function PlayersPage({ data, goHome }) {
  ...
  function headlineBadge(profile) {
    if (profile.bestFinish === 1) return 'DGLFC CHAMPION';
    if (profile.stateTrophyWins > 0) return 'STATE TROPHY';
    if (profile.redRoomCount > 0) return 'RED ROOM';
    if (profile.isRookie) return 'ROOKIE CARD';
    if (profile.rank) return `RANK ${profile.rank}`;
    return 'DGL TOUR';
  }

  function achievementLine(profile) {
    if (profile.bestFinish === 1) return 'DGLFC Champion Series';
    if (profile.stateTrophyWins > 0) return `${profile.stateTrophyWins} State Trophy Win${profile.stateTrophyWins === 1 ? '' : 's'}`;
    if (profile.redRoomCount > 0) return `${profile.redRoomCount} Red Room Round${profile.redRoomCount === 1 ? '' : 's'}`;
    if (profile.isRookie) return 'Official Rookie Card';
  ...
  return (
    <section className="red-room-page players-page card-gallery-page">
      ...
      <div className="red-page-hero annals-hero players-hero">
        <div className="red-page-copy">
          <p className="eyebrow">DGL PLAYER SET</p>
          <h1>Tour <span>Cards</span></h1>
          <p>Retro cards built from DGL photos, player bios, Red Room records, trophy history, and sportsbook odds.</p>
          <p className="red-subline">Tap and hold a card to reveal the stat back.</p>
        </div>
      </div>

      <div className="vintage-card-grid opc-card-grid">
        {profiles.length ? profiles.map(profile => {
          const cardColor = cardColorValue(profile.cardColor);
          const accentColor = cardHighlightColor(profile.cardColor);
          const title = profile.playerTitle || profile.nickname || headlineBadge(profile);
          return (
            <button
              className="flip-card opc-flip-card"
              key={profile.name}
              aria-label={`Flip ${profile.name} trading card`}
              style={{ '--card-color': cardColor, '--card-accent': accentColor }}
            >
              <div className="flip-card-inner opc-card-inner">
                <div className="card-face card-front opc-card-front">
                  <div className="opc-card-shell">
                    <div className="opc-topbar">
                      <span>DOJO GOLF LEAGUE</span>
                      <b>#{profile.cardNumber}</b>
                    </div>

                    <div className="opc-photo-window">
                      <PlayerPhoto name={profile.name} src={profile.photoUrl || profile.headshotUrl} className="opc-action-photo" />
                      <div className="opc-headshot-ring">
                        <PlayerPhoto name={profile.name} src={profile.headshotUrl || profile.photoUrl} className="opc-headshot" />
                      </div>
                      <img src="/dgl-logo.jpeg" alt="DGL" className="opc-logo-mark" />
                    </div>

                    <div className="opc-name-zone">
                      <div className="opc-team-stripe"></div>
                      <h2>{profile.name}</h2>
                      <p>{title ? `“${title}”` : headlineBadge(profile)}</p>
                    </div>

                    <div className="opc-stat-strip">
                      <span><small>RANK</small><b>{profile.rank || '—'}</b></span>
                      <span><small>PTS</small><b>{profile.points != null ? profile.points : '—'}</b></span>
                      <span><small>ODDS</small><b>{profile.odds ? formatOdds(profile.odds) : '—'}</b></span>
                    </div>

                    <div className="opc-achievement-ribbon">{achievementLine(profile)}</div>
                  </div>
                </div>

                <div className="card-face card-back opc-card-back">
                  <div className="opc-back-inner">
                    <div className="opc-back-header">
                      <span>DGL TOUR CARD #{profile.cardNumber}</span>
                      <h3>{profile.fullName || profile.name}</h3>
                      <p>{title || 'Official Player Card'}</p>
                    </div>

                    <div className="opc-back-columns">
                      <div><small>DOB</small><b>{profile.dob || '—'}</b></div>
                      <div><small>Debut</small><b>{profile.debutYear || '—'}</b></div>
                      <div><small>Height</small><b>{profile.height || '—'}</b></div>
                      <div><small>Hand</small><b>{profile.handedness || '—'}</b></div>
                      <div><small>Hometown</small><b>{profile.hometown || '—'}</b></div>
                      <div><small>Occupation</small><b>{profile.occupation || '—'}</b></div>
                    </div>

                    <table className="opc-stat-table">
                      <tbody>
                        <tr><th>2026 Rank</th><td>{profile.rank || '—'}</td><th>2026 Points</th><td>{profile.points != null ? profile.points : '—'}</td></tr>
                        <tr><th>DGLFC Apps</th><td>{profile.dglfcAppearances || 0}</td><th>Best DGLFC</th><td>{profile.bestFinish ? ordinal(profile.bestFinish) : '—'}</td></tr>
                        <tr><th>Red Room</th><td>{profile.redRoomCount || 0}</td><th>Best Net</th><td>{profile.bestNet ? formatNet(profile.bestNet) : '—'}</td></tr>
                        <tr><th>State Wins</th><td>{profile.stateTrophyWins || 0}</td><th>Odds</th><td>{profile.odds ? formatOdds(profile.odds) : '—'}</td></tr>
                      </tbody>
                    </table>

                    <div className="opc-back-notes">
                      <p><b>Favorite Course:</b> {profile.favoriteCourse || '—'}</p>
                      <p><b>Walk-Up Song:</b> {profile.walkUpSong || '—'}</p>
                    </div>

                    <p className="opc-card-bio">{profile.bio || 'Bio pending. Add notes in the Players tab to complete the back of this card.'}</p>
                  </div>
                </div>
              </div>
            </button>
