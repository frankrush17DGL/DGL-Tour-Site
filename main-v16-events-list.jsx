...
function parseFutureEvents(text) {
  ...
  // Future Events is intentionally a phone-friendly cross-tab:
  // Column A labels, events running across columns B, C, D...
  // Row labels expected: Week, Event, Date, Course, Time, Notes.
  const labelMap = {};
  rows.forEach((row, index) => {
    const label = normalizeHeader(row?.[0]);
    if (label) labelMap[label] = index;
  });

  const rowFor = (labels, fallback) => {
    for (const label of labels) {
      const normalized = normalizeHeader(label);
      if (labelMap[normalized] !== undefined) return labelMap[normalized];
    }
    return fallback;
  };

  const weekRow = rowFor(['Week'], 0);
  const eventRow = rowFor(['Event', 'Event #', 'Event Number'], 1);
  const dateRow = rowFor(['Date'], 2);
  const courseRow = rowFor(['Course'], 3);
  const timeRow = rowFor(['Time', 'Tee Time', 'Tee Times'], 4);
  const notesRow = rowFor(['Notes', 'Committed', 'Commitments', 'Number of Commits', 'Commits'], 5);
  ...
  for (let col = 1; col < maxCol; col++) {
    ...
    // Only show real events. Course is the safest required field.
    if (!course) continue;
  ...
  // Keep the same left-to-right order from the Future Events tab.
  // This avoids losing events because of Google date formatting quirks.
  return events;
}
