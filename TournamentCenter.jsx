...
const TOURNAMENT_CENTER_BUILD = 'TC 2026.09.13.1';
...
function useHistoricalCourseMetadata(rounds = []) {
  ...
  return useMemo(() => rounds.map(round => {
    ...
    const mappedCourse = eventNumber
      ? (courseMap[`${year}-${eventNumber}`] || courseMap[eventNumber] || '')
      : '';
    return (existingCourse || mappedCourse)
      ? { ...round, course: mappedCourse || existingCourse }
      : round;
  }), [rounds, courseMap]);
}
...
function TournamentEventPanel({
  ...
  const historyRows = useMemo(
    () => mergedRoundRows(rounds, historicalRounds, roundResults, results),
