...
const TOURNAMENT_CENTER_BUILD = 'TC 2026.08.17.1';
...
function useHistoricalCourseMetadata(rounds = []) {
  const [courseMap, setCourseMap] = useState({});
  const yearsKey = useMemo(() => Array.from(new Set(
    rounds.map(roundYear).filter(year => year >= 2021 && year <= new Date().getFullYear())
  )).sort().join(','), [rounds]);

  useEffect(() => {
    const years = yearsKey.split(',').map(Number).filter(Boolean);
    ...
    Promise.all(years.map(async year => {
      ...
      const baseUrl = `https://docs.google.com/spreadsheets/d/${DGL_SHEET_ID}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(sheetName)}`;

      const [eventResponse, courseResponse] = await Promise.all([
        fetch(`${baseUrl}&range=A2:AZ2&cacheBust=${Date.now()}`, { signal: controller.signal }),
        fetch(`${baseUrl}&range=A4:AZ4&cacheBust=${Date.now()}`, { signal: controller.signal })
      ]);

      if (!eventResponse.ok || !courseResponse.ok) {
        throw new Error(`Course row request failed for ${year}`);
      }

      const eventRow = parseCsvRows(await eventResponse.text())[0] || [];
      const courseRow = parseCsvRows(await courseResponse.text())[0] || [];
      const entries = {};
      ...
      for (let column = 0; column < Math.max(eventRow.length, courseRow.length); column += 1) {
        ...
        if (eventNumber && course) entries[`${year}-${eventNumber}`] = course;
      }

      return entries;
    }))
      .then(maps => setCourseMap(Object.assign({}, ...maps)))
      .catch(error => {
        if (error.name !== 'AbortError') {
          console.warn('Unable to load historical course metadata', error);
        }
      });
  ...
  return useMemo(() => rounds.map(round => {
    ...
    const existingCourse = round.course || round.courseName || round.golfCourse || round.venue || '';
    const mappedCourse = courseMap[`${year}-${eventNumber}`] || '';
    return (existingCourse || mappedCourse)
      ? { ...round, course: existingCourse || mappedCourse }
      : round;
  }), [rounds, courseMap]);
}
...
function TournamentEventPanel({
  ...
  const historyRowsWithCourses = useHistoricalCourseMetadata(historyRows);
