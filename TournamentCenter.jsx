...
function TournamentCenter({
  ...
  const projections = useMemo(
    () => projectedField(featuredEvent, sportsbook, historyRows),
    [featuredEvent, sportsbook, historyRows]
  );
  const headToHead = useMemo(() => {
    const names = committedNames(featuredEvent);
    return names.flatMap((player, index) => (
      names.slice(index + 1).map(opponent => headToHeadRecord(player, opponent, historyRows))
    )).filter(record => record.shared);
  }, [featuredEvent, historyRows]);
          ...
          {roundWeather.weather.length ? (
            ...
            <p className="tc-no-projections">
              {roundWeather.status === 'loading'
                ...
                : 'Forecast unavailable. Add Latitude and Longitude to this event, or use a full Maps link containing coordinates.'}
            </p>
          )}
        </section>
