...
import { AssetPhoto, canonicalName, fallbackData } from './core.jsx';

const EMPTY_WEATHER = Object.freeze([]);
const TOURNAMENT_CENTER_BUILD = 'TC 2026.09.13.2';
...
function committedPlayerName(value) {
  if (typeof value === 'string' || typeof value === 'number') return String(value).trim();
  if (!value || typeof value !== 'object') return '';
  return String(value.player || value.playerName || value.name || value.golfer || '').trim();
}

function committedNames(event = {}) {
  if (Array.isArray(event.committedPlayers) && event.committedPlayers.length) {
    return event.committedPlayers
      .map(committedPlayerName)
      .filter(Boolean)
      .filter((name, index, names) => (
        names.findIndex(candidate => canonicalName(candidate) === canonicalName(name)) === index
      ));
  }
...
const ROUND_COLLECTION_KEYS = ['rounds', 'results', 'history', 'records', 'data', 'rows'];

function roundRows(value, inherited = {}) {
  if (Array.isArray(value)) {
    return value.flatMap(item => roundRows(item, inherited));
  }
  if (!value || typeof value !== 'object') return [];

  const nestedCollections = ROUND_COLLECTION_KEYS
    .map(key => value[key])
    .filter(Array.isArray);
  const looksLikeResult = Boolean(roundPlayer(value)) || Number.isFinite(netPerformance(value));

  if (looksLikeResult || !nestedCollections.length) {
    return [{ ...inherited, ...value }];
  }

  const parentMetadata = Object.fromEntries(
    Object.entries({ ...inherited, ...value })
      .filter(([key]) => !ROUND_COLLECTION_KEYS.includes(key))
  );
  return nestedCollections.flatMap(collection => roundRows(collection, parentMetadata));
}

function mergedRoundRows(...sources) {
  const seen = new Set();
  return sources
    .flatMap(source => roundRows(source))
    .filter(round => {
      const player = canonicalName(roundPlayer(round));
      const performance = netPerformance(round);
      if (!player || !Number.isFinite(performance)) return false;
      const key = [roundEventKey(round), player, performance].join('|');
...
function TournamentEventPanel({
  ...
  const historyRows = useMemo(
    () => mergedRoundRows(
      ...
      roundResults,
      results,
      featuredEvent.historicalRounds,
      featuredEvent.rounds,
      featuredEvent.roundResults,
      featuredEvent.results
    ),
    [featuredEvent, rounds, historicalRounds, roundResults, results]
  );
