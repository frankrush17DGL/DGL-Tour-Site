...
const TOURNAMENT_CENTER_BUILD = 'TC 2026.09.02.2';
...
function useHistoricalCourseMetadata(rounds = [], historyMoments = []) {
  ...
  return useMemo(() => rounds.map(round => {
    ...
    const mappedCourse = eventNumber
      ? (historyCourseMap[`${year}-${eventNumber}`] || historyCourseMap[eventNumber] || courseMap[`${year}-${eventNumber}`] || courseMap[eventNumber] || '')
      : '';
    return (existingCourse || mappedCourse)
      ? { ...round, course: mappedCourse || existingCourse }
      : round;
  }), [rounds, courseMap, historyCourseMap]);
}
...
function headToHeadRecord(playerA, playerB, rounds = [], seasonYear = null, referenceDate = null) {
  ...
  events.forEach(({ results, round }) => {
    ...
    meetings.push({
      ...
      eventNumber: roundEventNumber(round),
...
function TournamentEventPanel({
  ...
  const openEventHistory = meeting => {
    ...
    const exact = historyMoments.find(moment => {
  ...
  return (
    <article className="card tc-shell" id={eventPosition === 1 ? 'events' : undefined}>
            ...
            <section className="tc-modal tc-event-modal" role="dialog" aria-modal="true" aria-label={`${meeting.event} history`} onMouseDown={event => event.stopPropagation()}>
              <div className="tc-modal-head">
                <div>
                  ...
                  <h3>{moment?.title || `${meeting.event} at ${meeting.course || 'Course unavailable'}`}</h3>
              ...
              <div className="tc-event-history-body">
                ...
                {moment?.body ? <p className="tc-event-story">{moment.body}</p> : <p className="tc-event-story">Historical details for this event are not yet recorded in the archive.</p>}
                {placements.length ? (
                  <div className="tc-event-results">
                    {placements.map(([place, name]) => <div key={place}><span>{place}</span><strong><PlayerLink name={name} onSelect={goPlayerProfile} /></strong></div>)}
                  </div>
                ) : null}
              </div>
            </section>
      ...
      <style>{`
        ...
        .tc-event-modal-backdrop{z-index:1100}.tc-event-modal{max-width:760px}.tc-event-history-body{overflow:auto;padding:20px;display:grid;gap:18px}.tc-event-history-body>img{width:100%;max-height:340px;object-fit:cover;border-radius:14px;border:1px solid rgba(240,199,94,.24)}.tc-event-facts{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:9px}.tc-event-facts span,.tc-event-results>div{display:grid;gap:4px;padding:12px;border-radius:11px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.06)}.tc-event-facts small,.tc-event-results span{color:#a99f8d;font-size:9px;font-weight:900;letter-spacing:.09em;text-transform:uppercase}.tc-event-story{margin:0;color:#ddd2c1;line-height:1.65}.tc-event-results{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:9px}.tc-event-results strong{color:#f0c75e}
