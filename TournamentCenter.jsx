...
function useRoundWeather(event, suppliedWeather = []) {
  ...
  useEffect(() => {
    ...
    if (!event?.course || !event?.date || !event?.time) return undefined;
    ...
    async function loadForecast() {
      try {
        ...
        let longitude = Number(event.longitude ?? event.lng ?? event.lon);
        ...
        const teeTime = new Date(`${event.date} ${event.time}`);
        ...
        const firstIndex = Math.max(0, hours.findIndex(hour => new Date(hour.time) >= teeTime));
        setWeather(hours.slice(firstIndex, firstIndex + 4));
        setStatus('ready');
      } catch (error) {
  ...
  }, [event?.course, event?.date, event?.time, event?.latitude, event?.longitude, suppliedWeather]);
...
function TournamentCenter({ events = [], sportsbook = [], rounds = [], weather = [] }) {
  const [selectedMatchup, setSelectedMatchup] = useState(null);
  const upcoming = Array.isArray(events) ? events : [];
  const featuredEvent = upcoming[0] || {};
  const secondaryEvents = upcoming.slice(1);
  const projections = useMemo(
    () => projectedField(featuredEvent, sportsbook, Array.isArray(rounds) ? rounds : []),
    [featuredEvent, sportsbook, rounds]
  );
  const headToHead = useMemo(() => {
    const names = committedNames(featuredEvent);
    return names.flatMap((player, index) => (
      names.slice(index + 1).map(opponent => headToHeadRecord(player, opponent, rounds))
      ...
      <div className="tc-intel-grid">
          ...
          {roundWeather.weather.length ? (
            ...
            <p className="tc-no-projections">
              {roundWeather.status === 'loading'
                ...
                : 'Forecast will appear when the event enters the forecast window and the course location is available.'}
            </p>
          )}
        </section>
        ...
        <section className="tc-matchups">
          ...
          {headToHead.length ? (
            ...
            <p className="tc-no-projections">
              Head-to-head records will appear when historical round results are supplied for the committed golfers.
            </p>
          )}
        </section>
      </div>
