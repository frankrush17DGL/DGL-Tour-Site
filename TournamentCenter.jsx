...
const TOURNAMENT_CENTER_BUILD = 'TC 2026.08.04.1';
...
function headToHeadRecord(playerA, playerB, rounds = []) {
  ...
  events.forEach(({ results, round }) => {
    ...
    meetings.push({
      ...
      eventWinner: eventWinners.length ? eventWinners.join(' / ') : 'Unavailable'
    });
  });
...
function projectedField(event, sportsbook, rounds = []) {
  ...
  const weights = enriched.map(player => {
    ...
    const formAdjustment = Number.isFinite(player.recentForm.value) && Number.isFinite(fieldForm)
      ? clamp((fieldForm - player.recentForm.value) / 12, -0.12, 0.12)
      : 0;
    const matchupRate = player.matchup.shared
      ? (player.matchup.wins + (player.matchup.ties * 0.5) + 5) / (player.matchup.shared + 10)
      : 0.5;
    const matchupAdjustment = clamp((matchupRate - 0.5) * 0.3, -0.06, 0.06);
...
function fieldStrengthHistory(projections = [], sportsbook = [], rounds = []) {
  const ratingByPlayer = new Map(
    (Array.isArray(sportsbook) ? sportsbook : [])
      .map(player => [canonicalName(player.player || player.name), Number(player.rating)])
      .filter(([player, rating]) => player && Number.isFinite(rating) && rating > 0)
  );
  ...
  rounds.forEach(round => {
    ...
    const playerKey = canonicalName(roundPlayer(round));
    if (!eventKey || !playerKey || !Number.isFinite(netPerformance(round))) return;
    if (!historicalEvents.has(eventKey)) historicalEvents.set(eventKey, new Set());
    historicalEvents.get(eventKey).add(playerKey);
  });

  const historicalAverages = [...historicalEvents.values()]
    .map(players => [...players].map(player => ratingByPlayer.get(player)).filter(Number.isFinite))
  ...
  const total = historicalAverages.length;
  ...
  return (
    <article className="card tc-shell" id={eventPosition === 1 ? 'events' : undefined}>
      <div className="tc-kicker-row">
        <div>
          <p className="eyebrow">Tournament Center • Event {eventPosition} of {eventCount}</p>
      ...
      <style>{`
        ...
        @media(max-width:760px){.tc-history-header{display:none}.tc-history-row{grid-template-columns:1fr 1fr}.tc-history-row>div:first-child,.tc-history-winner{grid-column:1/-1}.tc-history-field{text-align:left}.tc-history-row>div:not(:first-child) span{display:block}.tc-modal{max-height:90vh}.tc-modal-head{padding:16px}.tc-history-list{padding:10px 12px 16px}}
        @media(max-width:620px){.tc-shell{padding:14px}.tc-kicker-row{align-items:flex-start;flex-direction:column}.tc-hero{min-height:500px}.tc-course-photo,.tc-course-placeholder{height:500px}.tc-hero-copy{left:16px;right:16px;bottom:22px;max-width:none}.tc-countdown{left:14px;right:14px;top:14px;min-width:0}.tc-details{align-items:stretch;flex-direction:column}.tc-actions{justify-content:stretch}.tc-actions a{flex:1}.tc-hero-copy>strong{font-size:42px}.tc-mini-grid{grid-template-columns:1fr}.tc-subhead,.tc-upcoming-head{align-items:flex-start;flex-direction:column}.tc-weather-hours{grid-template-columns:repeat(2,minmax(0,1fr))}.tc-projection-label{align-items:stretch;flex-direction:column}.tc-projection-price{width:100%;flex-basis:auto;grid-template-columns:repeat(2,minmax(0,1fr))}.tc-projection-price span{white-space:normal}}
