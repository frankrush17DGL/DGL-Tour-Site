...
function parseCurrentStandings(sheet) {
  ...
  const players = blocks.map(block => {
    ...
    const points = Math.round(numericValuesFromRow(pointsRow, 4).reduce((sum, value) => sum + value, 0) * 100) / 100;
  ...
  return players.map((player, index) => {
    const rank = previousPoints !== null && Math.abs(player.points - previousPoints) < 0.0001
      ? previousRank
      : index + 1;
