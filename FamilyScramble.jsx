...
const champions = [
  { year: '2026', champion: 'Team Wishart', photo: winnersPhoto.src },
  { year: '2025', champion: "Team Sheehy (−1)", photo: '/family-scramble/2025.jpg' },
  { year: '2024', champion: "Team Leitch (−4)", photo: '/family-scramble/2024.jpg' },
  { year: '2023', champion: "Team Leitch", photo: '/family-scramble/2023.jpg' }
];
...
function FamilyScramblePage({ goHome }) {
  return (
    <div className="dfs-page">
      ...
      <section className="dfs-section dfs-winners" aria-labelledby="dfs-winners-heading">
        <a href={winnersPhoto.src} target="_blank" rel="noopener noreferrer" aria-label="View Team Wishart winners’ photo full size (opens in a new tab)">
          <img src={winnersPhoto.src} alt="Team Wishart, 2026 Dojo Family Scramble winners" />
        </a>
        <div>
          <p className="dfs-eyebrow">2026 Family Scramble Champions</p>
          <h2 id="dfs-winners-heading">Team Wishart</h2>
          <p>Scott Wishart • Grant Wishart • Wes Wishart • Chris Wishart</p>
          <strong>Gem Lake Hills • September 13, 2026</strong>
        </div>
      </section>
      ...
      <section className="dfs-section dfs-event-history" aria-labelledby="dfs-history-heading">
        <div className="dfs-section-heading">
          <p className="dfs-eyebrow">Family • Friends • Golf</p>
          <h2 id="dfs-history-heading">Family Scramble History</h2>
        </div>
        {scrambleHistory.map(event => (
          <article className="dfs-history-year" key={event.year} aria-labelledby={`dfs-history-${event.year}`}>
            <header className="dfs-history-head">
              <div>
                <h3 id={`dfs-history-${event.year}`}>{event.year}</h3>
                <p>{event.date} • {event.course}</p>
              </div>
              <div className="dfs-history-winner">
                <span>Champions</span>
                <strong>{event.winner}{event.score && ` (${event.score})`}</strong>
              </div>
            </header>
            <h4>Competing Teams</h4>
            <div className="dfs-history-teams">
              {event.teams.map(([players, score]) => (
                <div className="dfs-history-team" key={players}>
                  <span>{players}</span>
                  <strong aria-label={score ? `Score: ${score}` : 'Score not recorded'}>{score || '—'}</strong>
                </div>
              ))}
            </div>
            {event.teams.some(([, score]) => !score) && <p className="dfs-history-note">— Score not recorded.</p>}
            {event.contests.length > 0 && (
              <>
                <h4>Contest Winners & Prizes</h4>
                <div className="dfs-history-table-wrap" tabIndex={0} role="region" aria-label={`${event.year} contest winners and prizes`}>
                  <table className="dfs-history-table">
                    <thead><tr><th scope="col">Contest</th><th scope="col">Winner</th><th scope="col">Prize</th></tr></thead>
                    <tbody>{event.contests.map(([contest, winner, prize], index) => (
                      <tr key={`${contest}-${index}`}><th scope="row">{contest}</th><td>{winner}</td><td>{prize}</td></tr>
                    ))}</tbody>
                  </table>
                </div>
              </>
      ...
      <section className="dfs-section" aria-labelledby="dfs-gallery-heading">
        <div className="dfs-section-heading">
          <p className="dfs-eyebrow">2026 • Gem Lake Hills</p>
          <h2 id="dfs-gallery-heading">Scramble Memories</h2>
          <p className="dfs-gallery-note">Family. Friends. Golf. Select any photo to open the full-size image in a new tab.</p>
        </div>
        <div className="dfs-photo-grid">
          {eventPhotos.map(photo => (
            <a className="dfs-photo-link" key={photo.src} href={photo.src} target="_blank" rel="noopener noreferrer" aria-label={`${photo.label} (opens in a new tab)`}>
              <img src={photo.src} alt={photo.label} loading="lazy" decoding="async" />
              {photo.src === winnersPhoto.src && <span>Team Wishart • 2026 Champions</span>}
            </a>
          ))}
        </div>
      </section>
      ...
      <style>{`
        .dfs-history-year{margin-top:22px;padding:24px;border:1px solid rgba(255,216,103,.2);border-radius:18px;background:rgba(0,0,0,.22)}
        .dfs-history-head{display:flex;align-items:center;justify-content:space-between;gap:20px;padding-bottom:20px;border-bottom:1px solid rgba(255,216,103,.18)}
        .dfs-history-head h3{margin:0;color:#ffcf43;font-size:42px;letter-spacing:-.04em}
        .dfs-history-head p{margin:5px 0 0;color:#d8cebb;line-height:1.5;font-size:14px}
        .dfs-history-winner{padding:14px 18px;background:rgba(245,191,47,.1);border-radius:12px}
        .dfs-history-winner span{display:block;color:#f5bf2f;font-size:10px;letter-spacing:.15em;text-transform:uppercase;margin-bottom:5px}
        .dfs-history-winner strong{color:#fff;font-size:20px}
        .dfs-history-year h4{font-size:16px;margin:24px 0 12px;color:#fff}
        .dfs-history-team{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:13px 0;border-bottom:1px solid rgba(255,255,255,.08);font-size:14px;line-height:1.6}
        .dfs-history-team strong{color:#ffcf43;white-space:nowrap;min-width:36px;text-align:right;font-variant-numeric:tabular-nums}
        .dfs-history-note{font-size:12px;color:#c4b79f}
        .dfs-history-table-wrap{overflow-x:auto;border:1px solid rgba(255,255,255,.12);border-radius:12px}
        .dfs-history-table{width:100%;border-collapse:collapse;text-align:left;font-size:13px;min-width:440px}
        .dfs-history-table th,.dfs-history-table td{padding:13px 14px;border-bottom:1px solid rgba(255,255,255,.08);line-height:1.5}
        .dfs-history-table thead{background:rgba(245,191,47,.12);color:#ffcf43}
        .dfs-history-table tbody th{font-weight:600}.dfs-history-table tbody tr:last-child>*{border-bottom:0}
        .dfs-history-table-wrap:focus-visible{outline:2px solid #ffcf43;outline-offset:3px}
        @media(max-width:600px){.dfs-history-year{padding:16px}.dfs-history-head{align-items:flex-start;flex-direction:column;gap:14px}.dfs-history-winner{align-self:stretch}.dfs-history-team{font-size:13px}}

        .dfs-winners{display:grid;grid-template-columns:1.3fr 1fr;gap:26px;align-items:center}
        .dfs-winners>a{display:block;border-radius:16px;overflow:hidden;background:#100e0b}
        .dfs-winners img{display:block;width:100%;height:auto}
        .dfs-winners p:not(.dfs-eyebrow){color:#d8cebb;line-height:1.7}
        .dfs-winners strong{color:#f5c441;font-size:13px}
        .dfs-gallery-note{color:#d8cebb;line-height:1.6}
        .dfs-photo-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}
        .dfs-photo-link{position:relative;display:block;border-radius:14px;overflow:hidden;background:#181310;border:1px solid rgba(255,220,110,.18)}
        .dfs-photo-link img{display:block;width:100%;aspect-ratio:4/3;object-fit:cover}
        .dfs-photo-link span{position:absolute;bottom:0;left:0;right:0;padding:18px 12px 10px;color:#fff4cf;background:linear-gradient(transparent,rgba(0,0,0,.9));font-size:12px;font-weight:800}
        .dfs-photo-link:focus-visible,.dfs-winners>a:focus-visible{outline:3px solid #ffcf43;outline-offset:4px}
        @media(max-width:800px){.dfs-winners{grid-template-columns:1fr}.dfs-photo-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
        @media(max-width:420px){.dfs-photo-grid{grid-template-columns:1fr}}
        ...
        .dfs-champion-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.dfs-champion-card{position:relative;min-height:260px;border-radius:18px;overflow:hidden;background:#181310;border:1px solid rgba(255,220,110,.18)}.dfs-champion-card>img{width:100%;height:260px;object-fit:cover}.dfs-champion-placeholder{height:260px;display:grid;place-content:center;text-align:center;padding:20px;background:radial-gradient(circle at top,#5d3718,#170f0b 65%)}.dfs-champion-placeholder span{color:#f3c33e;font-size:42px;font-weight:950}.dfs-champion-placeholder strong{margin-top:8px;font-size:19px}.dfs-champion-placeholder small{margin-top:5px;text-transform:uppercase;letter-spacing:.13em;color:#c4b79f}.dfs-champion-copy{position:absolute;left:0;right:0;bottom:0;padding:16px;background:linear-gradient(0deg,rgba(0,0,0,.92),transparent);display:grid}.dfs-champion-copy span{color:#f5c441;font-weight:950}.dfs-champion-copy strong{font-size:16px}
